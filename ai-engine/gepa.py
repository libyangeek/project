...
تم دمج جداول الحركة الجانبية (Lateral Movement) وتعلم الأنماط المتقدم لعام 2026.
...
class SovereignOracleCore:
    ...
    def _init_db(self):
        ...
        # جدول التعلم الجيني من الأنماط (الذكاء الرباعي)
    ...
    def record_exploit(self, target, exploit_type, success, **kwargs):
        try:
            ...
            c.execute("""INSERT INTO exploits (ts, target, exploit_type, success, error, solution, improvement, severity, weight, agent, phase, execution_time) 
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        ...
        try:
            ...
            c.execute("""INSERT INTO lateral_movement (ts, source, target, method, success, error, token_used, agent) 
                      ...
                      (datetime.now().isoformat(), source, target, method, 1 if success else 0, 
                       kwargs.get('error'), kwargs.get('token'), kwargs.get('agent')))
            conn.commit()
            conn.close()
            return True
        except: return False
