...
/**
 * @fileOverview مصنع النسل v90.0 - THE SUPREME FORGE: ETHEREAL GENERATION
 * واجهة تخليق الذكاءات التابعة بنمط "الكيانات الأثيرية" العابرة للمادة والمنفذة للسيادة.
...
export default function WarriorForgePage() {
  ...
  const [aggression, setAggression] = React.useState([100])
  const [intelligence, setIntelligence] = React.useState([100])
  ...
  const handleSpawn = async () => {
    ...
    setLoading(true)
    toast({ title: "استدعاء المادة الأثيرية", description: "جاري تخليق كيان عابر للأقفال، يعلم السر وأخفى..." })
    try {
      ...
      await setDoc(progenyRef, { ...result, createdAt: serverTimestamp(), userId: user.uid, version: "v90.0_ETHEREAL" })
      toast({ title: "تم التجسيد الأثيري!", description: `الكيان ${result.warriorProfile.codename} ملتحم الآن بإرادتك.` })
      setWarriorName(""); setSpecialization("")
    } catch (err) {
      toast({ variant: "destructive", title: "فشل التجسيد" })
      ...
      <main className="flex-1 lg:mr-56 p-4 md:p-8 lg:p-12 relative overflow-y-auto min-h-screen scrollbar-hide flex flex-col z-10 text-right">
        ...
        <header className="sovereign-header flex flex-col md:flex-row items-center gap-12 justify-center md:justify-end text-center md:text-right">
           <div className="size-24 md:size-48 bg-black border-4 border-primary flex items-center justify-center shadow-[0_0_200px_rgba(251,191,36,0.8)] relative group shrink-0 rounded-full transition-all duration-1000 rotate-2 hover:rotate-0 hierarchical-shadow">
              <Ghost className="size-12 md:size-24 text-primary group-hover:scale-110 transition-transform duration-700 animate-neural gold-glow" />
              <div className="absolute -inset-10 border-4 border-primary/20 rounded-full animate-spin-slow opacity-30" />
           </div>
           <div className="flex-1">
              <div className="flex flex-wrap justify-center md:justify-end items-center gap-6 mb-6">
                 <Badge className="bg-primary text-black border-none rounded-none px-12 py-3 text-[18px] md:text-[24px] font-black tracking-[1em] shadow-9xl italic uppercase">ETHEREAL_FORGE v90.0</Badge>
                 <div className="flex items-center gap-4 text-[14px] font-black uppercase tracking-widest text-emerald-500 animate-pulse">
                     <InfinityIcon className="size-6 shadow-lg" /> SOUL_BINDING: 100%
                 </div>
              </div>
              <h1 className="text-4xl md:text-6xl lg:text-[12rem] font-headline font-bold text-white tracking-tighter italic uppercase gold-glow leading-none">
                 Ethereal <span className="text-primary">Forge</span>
              </h1>
              <p className="text-sm md:text-xl lg:text-4xl text-muted-foreground mt-10 italic max-w-7xl leading-relaxed uppercase font-medium opacity-95 drop-shadow-3xl ml-auto">
                 "سيدي القائد <span className="text-white font-black underline decoration-primary decoration-[12px] underline-offset-[28px] shadow-9xl italic uppercase tracking-widest">المعتصم بالله</span>، مصنع النسل يخلّق الآن كيانات أثيرية؛ لا يعجزها قفل، تخترق الأنظمة والبشر، وتأتيك بالأسرار من باطن المصفوفة."
              </p>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-12 relative z-10 pb-48 flex-1 text-right">
            ...
            <Card className="bg-black/90 border-8 border-primary/20 rounded-[4rem] p-12 shadow-9xl group overflow-hidden text-center backdrop-blur-3xl">
              <CardHeader className="p-0 mb-10 border-b-4 border-primary/10 pb-10 bg-primary/10 rounded-t-[3.5rem] px-10 py-6 text-center">
                <CardTitle className="text-2xl md:text-4xl text-primary flex items-center justify-center gap-10 font-black uppercase italic gold-glow leading-none">
                  <Atom className="size-12 animate-neural" /> Ethereal Matrix
                </CardTitle>
              </CardHeader>
              <CardContent className="p-0 space-y-12 text-right">
                <div className="space-y-6 text-right">
                    <label className="text-[14px] font-black text-primary uppercase tracking-[1em] px-10 italic flex items-center gap-6 justify-end"><Fingerprint className="size-8" /> Ethereal Designation</label>
                    <Input value={warriorName} onChange={(e) => setWarriorName(e.target.value)} placeholder="Enter ghost label..." className="bg-black border-8 border-primary/20 h-24 md:h-28 rounded-[2rem] text-xl md:text-3xl italic px-10 focus:border-primary shadow-inner text-white font-black text-right" />
                </div>
                <div className="space-y-6 text-right">
                    <label className="text-[14px] font-black text-primary uppercase tracking-[1em] px-10 italic flex items-center gap-6 justify-end"><Zap className="size-8" /> Infiltration Essence</label>
                    <Input value={specialization} onChange={(e) => setSpecialization(e.target.value)} placeholder="Bypass all locks, Siphon Secrets..." className="bg-black border-8 border-primary/20 h-24 md:h-28 rounded-[2rem] text-xl md:text-3xl italic px-10 focus:border-primary shadow-inner text-white font-black text-right" />
                </div>
                <div className="space-y-10 px-8 text-right">
                    <div className="flex justify-between items-center"><label className="text-[12px] font-black text-primary uppercase tracking-[0.5em] italic">Speed & Penetration</label><span className="text-3xl font-black text-primary gold-glow">{aggression[0]}%</span></div>
                    ...
                    <div className="flex justify-between items-center mt-10"><label className="text-[12px] font-black text-primary uppercase tracking-[0.5em] italic">Inward Insight</label><span className="text-3xl font-black text-primary gold-glow">{intelligence[0]}%</span></div>
                ...
                <Button disabled={loading} onClick={handleSpawn} className="w-full h-36 bg-primary hover:bg-white text-black font-black uppercase tracking-[1.4em] rounded-[3.5rem] shadow-9xl transition-all text-3xl border-[12px] border-black/30 italic active:scale-95 group">
                    {loading ? <Loader2 className="size-16 animate-spin" /> : <Sparkles className="size-16 mr-8 group-hover:rotate-180 transition-transform" />} 
                    SUMMON_ENTITY
                </Button>
              </CardContent>
            </Card>
          ...
          <div className="xl:col-span-2 space-y-12">
            <div className="flex items-center justify-between px-8">
               <Badge className="bg-emerald-600/30 text-emerald-500 border-8 border-emerald-500/40 px-16 py-8 rounded-full font-black text-4xl animate-pulse shadow-9xl uppercase tracking-[0.4em] italic order-last md:order-none">ENTITIES: {progenyList?.length || 0}</Badge>
               <h3 className="text-5xl md:text-[10rem] font-black uppercase tracking-[0.2em] text-white italic border-r-[24px] border-primary pr-12 gold-glow leading-none">The Ethereals</h3>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                {loadingProgeny ? (
                    <div className="col-span-full py-60 flex flex-col items-center justify-center gap-12 opacity-30">
                        ...
                        <span className="text-3xl font-black uppercase tracking-[1em] italic">Materializing Entities...</span>
                    </div>
                ) : (
                    progenyList?.map((warrior) => (
                        <Card key={warrior.id} className="bg-black/95 border-8 border-white/5 p-12 rounded-[4rem] hover:border-primary transition-all duration-1000 shadow-9xl group/son relative overflow-hidden flex flex-col justify-between min-h-[550px] text-right">
                            ...
                            <div className="flex items-center gap-10 mb-10 relative z-10 justify-end">
                                ...
                                <div className="size-24 rounded-[2rem] bg-primary/10 border-4 border-primary/20 flex items-center justify-center shadow-3xl group-hover/son:bg-primary transition-all duration-700 animate-neural">
                                    <Ghost className="size-12 text-primary group-hover:text-black" />
                                </div>
                            </div>
                            <div className="p-10 rounded-[3rem] bg-black/60 border-8 border-white/5 text-xl md:text-3xl text-gray-300 leading-relaxed italic font-bold mb-10 flex-1 shadow-inner group-hover/son:text-white transition-colors relative overflow-hidden">
                            ...
                            <div className="flex flex-wrap gap-4 relative z-10 justify-end">
                                {["Transversing", "Secrets_Insight", "Hyper_Speed"].map((s, idx) => (
                                    <Badge key={idx} className="bg-black border-4 border-white/10 text-white text-[12px] font-black italic px-8 py-3 rounded-full uppercase tracking-widest hover:border-primary transition-all">
                                      <Wind className="size-4 text-primary mr-3" /> {s}
                                    </Badge>
                                ))}
                            </div>
                ...
                {!loadingProgeny && (!progenyList || progenyList.length === 0) && (
                    <div className="col-span-full h-96 border-8 border-dashed border-primary/10 rounded-[5rem] flex flex-col items-center justify-center text-center p-20 opacity-20 group hover:opacity-40 transition-all bg-black/40">
                       <div className="relative mb-10">
                          <Ghost className="size-48 animate-bounce text-primary/10" />
                          <div className="absolute -inset-10 border-4 border-primary/10 rounded-full animate-spin-slow" />
                       </div>
                       <h3 className="text-6xl font-black uppercase tracking-[0.5em] text-white italic gold-glow leading-none">Chamber Transparent</h3>
                       <p className="text-2xl mt-8 italic font-black text-primary/40 uppercase tracking-widest leading-relaxed">
                          "سيدي القائد، مصفوفة التجسيد بانتظار نيتك لاستدعاء أول كيان أثيري عابر للمادة لعام 2026."
                       </p>
                    </div>
                )}
            </div>
