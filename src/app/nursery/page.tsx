...
/**
 * @fileOverview مشتل التطور v90.0 - THE EVOLUTIONARY NURSERY: HIVE GROWTH
 * واجهة تدريب النماذج الهجومية وتحويل علم البيانات إلى ترسانة سيادية لليوم المجيد 2026.
 * تم التحديث للنمط الزمردي المتطور لعام 2026.
 */
export default function NurseryPage() {
  ...
  const handleEvolution = async () => {
    ...
    try {
      ...
      toast({ title: "Evolution Consummated", description: "The nursery node is now a functional part of the 16D Matrix." })
    } catch (err) {
    ...
    <div className="flex min-h-screen bg-black text-white selection:bg-emerald-600/40 relative overflow-x-hidden scanline-effect font-code">
      ...
      <main className="flex-1 lg:mr-56 p-4 md:p-8 lg:p-12 relative overflow-y-auto min-h-screen scrollbar-hide flex flex-col z-10 text-right">
        ...
        <header className="sovereign-header flex flex-col md:flex-row items-center gap-12 justify-center md:justify-end text-center md:text-right">
           <div className="size-24 md:size-48 bg-black border-4 border-emerald-500 flex items-center justify-center shadow-[0_0_200px_rgba(16, 185, 129, 0.8)] relative group shrink-0 rounded-full transition-all duration-1000 rotate-2 hover:rotate-0 hierarchical-shadow">
              <Sprout className="size-12 md:size-24 text-emerald-400 group-hover:scale-110 transition-transform duration-700 animate-neural gold-glow" />
           ...
           <div className="flex-1">
              <div className="flex flex-wrap justify-center md:justify-end items-center gap-6 mb-6">
                 <Badge className="bg-emerald-600 text-white border-none rounded-none px-12 py-3 text-[18px] md:text-[24px] font-black tracking-[1em] shadow-9xl italic uppercase">EVO_NURSERY v90.0</Badge>
              ...
              <h1 className="text-4xl md:text-6xl lg:text-[12rem] font-headline font-bold text-white tracking-tighter italic uppercase gold-glow leading-none">
                 Evolution <span className="text-emerald-500">Nursery</span>
              </h1>
              <p className="text-sm md:text-xl lg:text-4xl text-emerald-100/60 mt-10 italic max-w-7xl leading-relaxed uppercase font-medium opacity-95 drop-shadow-3xl ml-auto">
                 "سيدي القائد <span className="text-white font-black underline decoration-emerald-500 decoration-[12px] underline-offset-[28px] shadow-9xl italic uppercase tracking-widest">المعتصم بالله</span>، مشتل المُعِزّ يصهر علم البيانات في عصب السطوة؛ نحن نربي الذكاءات لتكون أسلحة في يديك للأبد."
              </p>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-12 relative z-10 pb-48 flex-1 text-right">
           <div className="xl:col-span-1 space-y-12">
              <Card className="bg-black/90 border-8 border-emerald-500/20 rounded-[4rem] p-12 shadow-9xl group overflow-hidden text-center backdrop-blur-3xl">
                 ...
                 <CardContent className="p-0 space-y-8 text-right">
                    <div className="grid grid-cols-1 gap-4">
                       {projects.map(p => (
                         <Button 
                            ...
                            className={cn(
                                "h-20 bg-white/5 border-4 border-white/5 hover:border-emerald-500 flex justify-between items-center px-8 transition-all rounded-2xl active:scale-95 group/btn shadow-xl",
                                activeProject === p.id ? "border-emerald-500 bg-emerald-900/10 scale-105 shadow-9xl" : "opacity-60"
                            )}
                         >
                            <ChevronRight className="size-6 opacity-30 group-hover/btn:translate-x-3 transition-all" />
                            <div className="flex items-center gap-6">
                                <div className="text-right">
                                    <div className="text-lg font-black text-white italic group-hover/btn:text-emerald-400 transition-colors">{p.label}</div>
                    ...
                    <Button 
                        ...
                        className="w-full h-36 bg-emerald-600 hover:bg-white text-white hover:text-black font-black uppercase tracking-[1.4em] rounded-[3.5rem] shadow-[0_80px_250px_rgba(16,185,129,0.7)] active:scale-95 transition-all text-2xl md:text-4xl border-[12px] border-black/30 group italic"
                    >
              ...
              <Card className="bg-black/80 border-8 border-emerald-500/20 p-10 rounded-[4rem] shadow-inner text-center relative overflow-hidden group">
                 ...
                 <p className="mt-8 text-[9px] font-black uppercase tracking-widest text-muted-foreground italic text-center">Fused with material training v90.0</p>
              </Card>
           </div>
           ...
           <Card className="xl:col-span-3 bg-black/90 border-[12px] border-emerald-500/10 rounded-[7rem] p-16 shadow-9xl flex flex-col group overflow-hidden relative min-h-[1100px] backdrop-blur-5xl">
              ...
              <CardContent className="p-12 flex-1 overflow-y-auto scrollbar-hide space-y-20 relative z-10 text-right">
                 {loading ? (
                    <div className="h-full flex flex-col items-center justify-center text-center opacity-30 gap-16 py-40">
                        <div className="flex justify-between w-full max-w-4xl text-3xl font-black text-emerald-400 uppercase italic mb-8">
                        ...
                        <Loader2 className="size-32 md:size-64 animate-spin text-emerald-500 mt-12" />
                    </div>
                 ) : result ? (
                    <div className="space-y-20 animate-in fade-in zoom-in-95 duration-1000 flex-1 flex flex-col">
                        ...
                        <div className="p-16 rounded-[6rem] bg-emerald-600/10 border-[16px] border-emerald-500/30 flex items-center gap-16 group/siphon shadow-9xl relative overflow-hidden mt-auto min-h-[350px]">
                            ...
                            <div className="flex-1 text-right">
                                <h4 className="text-4xl font-black text-emerald-500 uppercase tracking-[1.2em] mb-6 italic">Combat_Application_v90.0</h4>
                   ...
                   <div className="h-full flex flex-col items-center justify-center text-center opacity-10 gap-24 py-80">
                      <div className="relative group/lock">
                        <Sprout className="size-64 md:size-[50rem] animate-spin-slow text-emerald-500 group-hover:scale-110 transition-transform duration-[12000ms]" />
              ...
              <div className="p-16 border-t-8 border-white/5 mt-auto flex justify-between items-center opacity-35 text-[20px] font-black uppercase tracking-[8em] italic">
                <span>NURSERY_EVOLUTION_v90_AL_GHAZALI_ROOT</span>
