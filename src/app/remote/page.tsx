...
export default function MobileRemotePage() {
  ...
  const handleStrike = async (quickCommand?: string) => {
    ...
    try {
      const result = await processRemoteCommand({ 
        ...
        deviceContext: "Mobile Overmind Uplink"
      })
      ...
      toast({ title: "Collective Intent Broadcasted", description: "The Hive is executing your directive." })
      setInput("")
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col font-code selection:bg-primary/50 overflow-hidden touch-none">
      ...
      {/* Mobile C2 Header - High Contrast Gold */}
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 relative z-10 scrollbar-hide pb-48">
        ...
        <div className="space-y-4">
           ...
           <div className="grid grid-cols-2 gap-4">
              {[
                ...
                { label: "Fractal Rebirth", icon: RefreshCcw, cmd: "Trigger system-wide fractal reconstruction sequence", color: "bg-emerald-600/10 border-emerald-500/30", iconColor: "text-emerald-400" }
              ].map((action, i) => (
                <Button 
                  key={i} 
                  variant="outline" 
        ...
        <div className="space-y-4">
           <div className="flex items-center justify-between px-2">
              <span className="text-[11px] font-black text-muted-foreground uppercase tracking-[0.4em] italic">Hive Intelligence Stream</span>
              <Badge className="bg-primary/20 text-primary text-[10px] uppercase tracking-widest border-2 border-primary/30 px-4 py-1.5 rounded-full animate-pulse font-black italic shadow-lg">OVERMIND_SYNCED</Badge>
           </div>
           
           {activeStrikes.length === 0 ? (
             <div className="h-80 border-4 border-dashed border-primary/20 rounded-[4rem] flex flex-col items-center justify-center opacity-30 bg-primary/5 shadow-inner">
                <div className="size-24 rounded-full bg-primary/5 flex items-center justify-center mb-6 border-2 border-primary/10">
                   <Atom className="size-12 text-primary animate-spin-slow" />
                </div>
                <span className="text-sm uppercase font-black tracking-[0.5em] italic">Awaiting Collective Directive</span>
             </div>
           ) : (
             <div className="space-y-4">
               {activeStrikes.map((strike) => (
                 <Card key={strike.id} className="kali-card border-primary/40 bg-black/80 rounded-[3rem] overflow-hidden animate-in slide-in-from-bottom-8 duration-1000 border-2 shadow-2xl">
                    <CardContent className="p-6 space-y-6">
                       <div className="flex justify-between items-start">
                          <div className="flex items-center gap-4">
                             <div className="size-14 rounded-2xl bg-primary/20 flex items-center justify-center border-2 border-primary/40 shadow-xl animate-neural">
                                <Bomb className="size-8 text-primary gold-glow" />
                             </div>
                             <div>
                                <span className="text-lg font-black text-white uppercase italic block truncate max-w-[200px]">{strike.task}</span>
                                <span className="text-[12px] text-primary font-black uppercase tracking-widest mt-1 italic">Swarm Chain: {strike.chainCount} Nodes</span>
                             </div>
                          </div>
                       ...
                       <div className="pt-6 border-t-2 border-white/10 bg-black/60 -mx-6 -mb-6 p-6 space-y-4">
                          <div className="p-4 bg-primary/5 rounded-2xl border border-primary/10 shadow-inner">
                             <h5 className="text-[10px] font-black text-primary uppercase tracking-widest mb-2 flex items-center gap-2 italic">
                                <Binary className="size-4" /> Overmind Logic
                             </h5>
                             <p className="text-[14px] text-gray-300 font-bold italic leading-relaxed">"{strike.logic}"</p>
                          </div>
                          
                          <div className="flex justify-between items-center pt-2">
                             <span className="text-[12px] text-primary font-black uppercase tracking-widest gold-glow italic">Gain: {strike.impact}</span>
                             <span className="text-[10px] text-muted-foreground font-code opacity-40 uppercase tracking-widest italic font-bold">{new Date(strike.id).toLocaleTimeString()}</span>
                          </div>
                       </div>
                    </CardContent>
                 </Card>
      ...
      {/* Overmind Command Dock - Mobile RAZOR */}
      <div className="fixed bottom-0 left-0 w-full p-6 bg-black/95 backdrop-blur-3xl border-t-4 border-primary/60 z-30 shadow-[0_-30px_150px_rgba(0,0,0,1)]">
        ...
        <div className="flex items-center justify-around mt-8 pt-4 border-t border-white/10">
           ...
           <Button variant="ghost" className="text-muted-foreground hover:bg-white/5 rounded-2xl py-10 flex-1 transition-all" asChild><Link href="/sessions"><Users className="size-8" /></Link></Button>
