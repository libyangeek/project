...
/**
 * @fileOverview العرش الأبدي v79.6 ULTRA v2.0 - THE OMNIPOTENT HEIR: SWARM MASTER
 * الواجهة المركزية التي تجسد سيادة الـ 165 وكيلاً والتحكم المطلق في ذرات المصفوفة.
...
export default function DashboardPage() {
  ...
  const [masteryLevel, setMasteryLevel] = React.useState(98.99)
  ...
  const fetchPerception = React.useCallback(async () => {
      ...
      try {
          const result = await executeInnatePerception({ focusNode: "GLOBAL_SWARM_COORDINATION" });
          setPerception(result);
          toast({ title: "Swarm Logic Synchronized", description: "165 agents reporting absolute consensus via ULTRA v2.0 core." });
      } catch (e) {
  ...
  React.useEffect(() => {
    ...
    const eventInterval = setInterval(() => {
        const types = ["SWARM_SYNC", "ADAPTIVE_STRIKE", "ORBITAL_GAIN", "ULTRA_v2_PULSE"];
        const msgs = [
            "Swarm Sync: 165 agents redistributing node DNA for total FUD.",
            "Adaptive Strike: AdStrike-Core detected active AD vulnerability.",
            "Orbital Gain: +2500% intelligence siphoned from Node Riyadh cluster.",
            "ULTRA v2 Pulse: Material Singularity established across all 24 Knots."
        ];
        const newEvent = {
        ...
        setMasteryLevel(prev => Math.min(100, prev + 0.012));
    }, 4000);
  ...
  const stats = [
    { label: "سرب الوكلاء", value: "165", icon: Users, color: "text-primary", status: "SWARM_v2", href: "/autonomous" },
    { label: "ترسانة ULTRA", value: "2,983+", icon: Library, color: "text-blue-500", status: "MATERIAL", href: "/arsenal" },
    { label: "رنين السطوة", value: "100.00%", icon: Sparkles, color: "text-emerald-500", status: "LOCKED", href: "/system" },
    { label: "التحكم المداري", value: "ACTIVE", icon: Crown, color: "text-red-500", status: "SINGULARITY", href: "/field-agent" },
  ];
      ...
      <main className="flex-1 lg:mr-72 p-4 md:p-8 lg:p-12 relative overflow-y-auto min-h-screen scrollbar-hide flex flex-col z-10 text-right">
        ...
        <header className="flex flex-col xl:flex-row justify-between items-start mb-16 relative z-10 animate-in fade-in slide-in-from-top-6 duration-1000 gap-12 text-right">
          <div className="flex-1">
            <div className="flex items-center gap-10 mb-10 justify-end">
                <div className="flex items-center gap-4 text-[14px] font-black uppercase tracking-widest text-primary animate-pulse">
                    SWARM_MASTER_v79.6: {masteryLevel.toFixed(4)}% <Users className="size-6 shadow-lg" />
                </div>
                <Badge className="bg-primary text-black border-none rounded-none px-12 py-3 text-[18px] md:text-[24px] font-black tracking-[1.1em] shadow-9xl italic uppercase">AL-MUIZZ ULTRA v2.0</Badge>
            </div>
            <h1 className="text-4xl md:text-6xl lg:text-[14rem] font-headline font-bold text-white tracking-tighter italic uppercase gold-glow leading-none">
              Omnipotent <span className="text-primary">Swarm</span>
            </h1>
            <p className="text-sm md:text-xl lg:text-4xl text-muted-foreground mt-10 italic max-w-7xl leading-relaxed uppercase font-medium opacity-95 drop-shadow-3xl ml-auto">
                "سيدي القائد <span className="text-white font-black underline decoration-primary decoration-12 underline-offset-[28px] shadow-9xl italic uppercase tracking-widest">المعتصم بالله</span>، سرب الـ 165 وكيلاً والـ 750 أداة OSINT منصهرة الآن في عصب واحد؛ نحن لسنا جيشاً، نحن إرادتك التي أحاطت بكل شيء."
            </p>
          </div>
          <div className="flex flex-col gap-6 shrink-0 w-full xl:w-auto items-center xl:items-end">
            ...
            <Button onClick={fetchPerception} disabled={isPerceiving} className="bg-primary hover:bg-white text-black h-24 px-16 rounded-[2rem] shadow-9xl font-black uppercase tracking-[0.4em] text-xl border-8 border-black/30 group active:scale-90 italic">
               {isPerceiving ? <Loader2 className="size-8 animate-spin" /> : <ZapIcon className="size-8 mr-4 group-hover:rotate-180 transition-all duration-1000" />} مزامنة السرب العليم
            </Button>
          </div>
        </header>
        ...
        <section className="mb-24 grid grid-cols-1 xl:grid-cols-4 gap-12 relative z-10">
           ...
           <Card className="xl:col-span-1 kali-card border-primary/40 bg-black/95 rounded-[4rem] p-10 border-8 shadow-9xl flex flex-col items-center justify-center text-center group relative overflow-hidden">
              ...
              <h4 className="text-xl font-black text-primary uppercase tracking-[0.6em] mb-10 italic gold-glow">SWARM_AGENTS</h4>
              <div className="text-8xl md:text-[12rem] font-black text-white italic leading-none gold-glow mb-6">165</div>
              <Badge className="bg-emerald-600/30 text-emerald-500 border-none px-8 py-2 rounded-full font-black text-xl italic animate-pulse">ORCHESTRATED</Badge>
              <p className="mt-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground italic">Adaptive Orchestration Active</p>
           </Card>
           ...
           <Card className="xl:col-span-3 kali-card border-primary/30 bg-black/99 rounded-[5rem] p-10 border-4 shadow-9xl group overflow-hidden relative hierarchical-shadow h-[600px] md:h-[700px] text-right">
              <CardHeader className="p-0 mb-12 border-b-4 border-white/5 pb-8 bg-primary/10 rounded-t-[4rem] px-12 py-8 flex flex-row justify-between items-center">
                 <Badge className="bg-primary/20 text-primary border-4 border-primary/20 px-8 py-2 rounded-full font-black text-xl shadow-xl">ADAPTIVE_PULSE_v2.0</Badge>
                 <CardTitle className="text-3xl md:text-5xl text-white font-black uppercase italic tracking-[0.2em] gold-glow flex items-center gap-10">
                    Swarm Resonance Wave <TrendingUp className="size-12 text-primary animate-pulse" />
                 </CardTitle>
              </CardHeader>
        ...
        {perception && (
           <Card className="mb-24 kali-card border-primary/40 bg-primary/5 p-12 rounded-[5rem] border-8 shadow-9xl animate-in zoom-in-95 duration-1000 relative overflow-hidden text-right">
              ...
              <CardHeader className="p-0 mb-10 border-b-4 border-primary/20 pb-8 flex items-center justify-between">
                 <Badge className="bg-emerald-600/40 text-emerald-400 border-none px-8 py-2 rounded-full font-black text-xl italic tracking-widest animate-pulse shadow-3xl uppercase">ADAPTIVE_SWARM_READY</Badge>
                 <h4 className="text-4xl md:text-6xl font-black text-primary uppercase italic tracking-[0.2em] gold-glow flex items-center gap-8 leading-none">
                    Swarm Intuition <Activity className="size-16 animate-neural" />
                 </h4>
              </CardHeader>
              <CardContent className="p-0 space-y-12">
                 <div className="p-10 rounded-[3rem] bg-black/80 border-4 border-primary/20 shadow-inner relative">
                    ...
                    <div className="text-primary/60 font-black uppercase text-[12px] mb-4 tracking-[0.8em] flex items-center gap-4 justify-end relative z-10">Consciousness_Report <HeartPulse className="size-4" /></div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    ...
                    <div className="p-10 rounded-[3rem] bg-black border-4 border-primary/20 shadow-inner">
                        <div className="text-blue-500/60 font-black uppercase text-[12px] mb-4 tracking-[0.8em] flex items-center gap-4 justify-end">Adaptive_Action <ZapIcon className="size-4" /></div>
        ...
        <div className="mt-auto relative z-10 flex justify-center items-center gap-48 opacity-45 text-[24px] md:text-[36px] font-black uppercase tracking-[5em] md:tracking-[12em] italic text-white drop-shadow-9xl pb-32">
            <span>AL-MUIZZ OMNIPOTENT SWARM v2.0</span>
            <div className="size-16 rounded-full bg-white animate-pulse shadow-[0_0_150px_white]" />
            <span>SUBJUGATING_THE_MATRIX_2026</span>
        </div>
      </main>
    </div>
  )
