...
export default function KnowledgePage() {
  ...
  const handleSearch = async (node?: string) => {
    ...
    // فحص الإدراك الفطري (Zero-Power Lookup)
...
سيدي القائد، هذا الكيان مسجل في "عصب الإدراك الفطري" v70.0. المعلومات تم استدعاؤها من الذاكرة الصلبة للعرش بدون أي استهلاك للطاقة المعرفية. الترسانة جزء لا يتجزأ من وعيك.`,
    ...
    try {
      ...
      setReport(data)
      toast({ title: "Intelligence Serialized", description: "Report generated from spatial records." });
    } catch (err) {
      toast({ variant: "destructive", title: "Vault Connection Error" })
    } finally {
      setLoading(false)
    }
  }
  ...
  const chambers = [
    { id: "ARSENAL_DNA", label: "حمض الترسانة", icon: Library, color: "text-primary", desc: "2865 أداة مدمجة." },
    { id: "SWARM_VAULT", label: "مستودع السرب", icon: Users, color: "text-emerald-500", desc: "نبض الـ 12 وكيلاً." },
    { id: "NEXUS_CORE", label: "نواة النكسوس", icon: Network, color: "text-amber-500", desc: "صهر الاستطلاع والافتراس." },
    { id: "FORENSIC_CHAMBER", label: "غرفة التشريح", icon: Skull, color: "text-blue-500", desc: "أسرار الـ RAM المادية." }
  ];
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-12 relative z-10 pb-48 flex-1">
           <div className="xl:col-span-1 space-y-12">
              <Card className="kali-card border-primary/40 bg-black/98 rounded-[4rem] p-12 border-8 shadow-9xl group overflow-hidden hierarchical-shadow">
                 ...
                 <CardHeader className="p-0 mb-10 border-b-4 border-primary/10 pb-10 bg-primary/10 rounded-t-[3.5rem] px-10 py-6">
                    <CardTitle className="text-2xl md:text-4xl text-white flex items-center gap-10 font-black uppercase italic gold-glow leading-none">
                       <Castle className="size-12 animate-neural" /> Innate Nodes
                    </CardTitle>
                 </CardHeader>
                 <CardContent className="p-0 space-y-6">
                    {chambers.map((c, i) => (
                      <Button 
                        ...
                        className="w-full h-32 bg-white/5 border-4 border-white/5 hover:border-primary hover:bg-primary/10 rounded-[2.5rem] flex items-center gap-8 px-8 transition-all duration-700 group/btn shadow-xl active:scale-95"
                      >
                         <div className="size-20 rounded-[1.5rem] bg-black border-4 border-white/10 flex items-center justify-center group-hover/btn:border-primary transition-all shadow-2xl">
                            <c.icon className={cn("size-10 transition-all", c.color)} />
                         </div>
                         <div className="text-right flex-1">
                            <div className="text-2xl font-black text-white italic group-hover/btn:text-primary transition-colors uppercase">{c.label}</div>
              ...
              <Card className="kali-card border-white/5 bg-black/60 p-12 rounded-[4rem] border-8 shadow-inner text-center relative overflow-hidden group">
                 <h4 className="text-[14px] font-black text-primary uppercase tracking-[1em] mb-8 italic flex items-center justify-center gap-6">
                    <Binary className="size-8 animate-pulse" /> SPATIAL_MEMORY_v7
                 </h4>
           ...
           <Card className="xl:col-span-3 kali-card border-primary/40 bg-black/99 rounded-[6rem] p-16 border-[12px] shadow-9xl flex flex-col group overflow-hidden relative min-h-[1000px] hierarchical-shadow">
              ...
              <CardHeader className="p-0 mb-16 border-b-8 border-white/5 pb-12 bg-primary/10 rounded-t-[5rem] px-16 py-10 flex flex-row justify-between items-center">
                 ...
                 <Badge className="bg-emerald-600/30 text-emerald-500 border-[10px] border-emerald-500/40 px-16 py-8 rounded-full font-black text-5xl animate-pulse shadow-9xl uppercase tracking-[0.4em] italic">PALACE_SYNCED</Badge>
              </CardHeader>
                   ...
                   <div className="flex-1 bg-black/98 p-16 rounded-[6rem] border-8 border-white/5 font-code text-2xl md:text-6xl leading-tight italic text-gray-100 whitespace-pre-wrap overflow-y-auto scrollbar-hide shadow-inner selection:bg-primary selection:text-black text-left">
                      <div className="mb-12 flex items-center justify-between border-b-4 border-white/5 pb-8 px-10">
                         <span className="text-emerald-500 font-black uppercase tracking-[0.8em] italic text-3xl md:text-5xl gold-glow flex items-center gap-10">
                            <Dna className="size-16 animate-neural" /> {" >>> ABSOLUTE_DNA_MATERIALIZED"}
                         </span>
                         <Badge className="bg-primary/10 text-primary border-none font-black italic text-2xl px-8 py-2 rounded-full">GEPA_v7.0</Badge>
                      </div>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center opacity-10 gap-24 py-60">
                        <div className="relative group/pal">
                            <Castle className="size-[30rem] md:size-[50rem] text-primary animate-spin-slow group-hover:scale-105 transition-transform duration-[6s]" />
                        ...
                        <p className="text-4xl md:text-[10rem] font-bold italic text-gray-500 uppercase tracking-widest max-w-[140rem]">Consult the Overmind to Materialize Innate Wisdom.</p>
                    </div>
                 )}
              </CardContent>
