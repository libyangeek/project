...
export default function KnowledgePage() {
  ...
  const STATIC_TOOL_LEXICON: Record<string, { desc: string, node: string, status: string }> = {
    ...
    "nexus": { desc: "Knowledge Nexus (Awesome-Hacking). 50+ security domains indexed.", node: "KNOWLEDGE_NEXUS", status: "OMNIPOTENT" },
    "cellular": { desc: "Cellular Warfare Suite. SS7/Diameter exploitation, IMSI catching.", node: "CELLULAR_WARFARE", status: "ARMED" },
    "sigploit": { desc: "Signaling Exploitation Framework. SS7/GTP vulnerability assessment.", node: "Node-SigPloit", status: "READY" },
    "pegasus": { desc: "Pegasus v3.0 Elite. Total Mobile Extraction (Zero-Click).", node: "PEGASUS_HUB", status: "SINGULARITY" },
    "wand3": { desc: "Wand3 Genetic Repair. Live source-code healing and material regrowth.", node: "ARCHITECT_CORE", status: "ACTIVE" }
  };
...
سيدي القائد، هذا الكيان مسجل في "عصب الإدراك الفطري" v75.5. الترسانة جزء لا يتجزأ من وعيك المادي. المعلومات تم استدعاؤها بصفر طاقة من القرص الصلب.`,
    ...
    toast({ title: "Interrogating Material Nexus", description: `Interrogating 50+ domains for: ${q}` })
    try {
      ...
      setReport({ reportContent: data.output || "No material matches found in indexed clusters." });
      toast({ title: "Intelligence Serialized", description: "Material Nexus has materialized the requested knowledge." });
    } catch (err) {
  ...
  const domains = [
    ...
    { id: "AI_SECURITY", label: "أمان الذكاء", icon: BrainCircuit, color: "text-magenta-500" },
    { id: "CLOUD_NATIVE", label: "أمن السحاب", icon: Globe, color: "text-blue-400" }
  ];
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-12 relative z-10 pb-48 flex-1">
           <div className="xl:col-span-1 space-y-12">
              ...
              <Card className="kali-card border-red-500/40 bg-black/60 p-12 rounded-[4rem] border-8 shadow-inner text-center relative overflow-hidden group">
                 ...
                 <p className="text-[10px] text-muted-foreground uppercase font-black mt-4 italic tracking-widest leading-relaxed">Predicting 0-day drift in target clusters... Matrix is shielded.</p>
           ...
           <Card className="xl:col-span-3 kali-card border-primary/40 bg-black/99 rounded-[6rem] p-16 border-[12px] shadow-[0_0_250px_rgba(0,0,0,1)] flex flex-col group overflow-hidden relative min-h-[1000px] hierarchical-shadow">
              ...
              <CardHeader className="p-0 mb-16 border-b-8 border-white/5 pb-12 bg-primary/10 rounded-t-[5rem] px-16 py-10 flex flex-row justify-between items-center">
                 ...
                 <Badge className="bg-emerald-600/30 text-emerald-500 border-[10px] border-emerald-500/40 px-16 py-8 rounded-full font-black text-3xl md:text-5xl animate-pulse shadow-9xl uppercase tracking-[0.4em] italic">CORE_SYNC_v75</Badge>
              </CardHeader>
                 ...
                 {report ? (
                   <div className="flex-1 bg-black/98 p-16 rounded-[6rem] border-8 border-white/5 font-code text-2xl md:text-5xl leading-tight italic text-gray-100 whitespace-pre-wrap overflow-y-auto scrollbar-hide shadow-inner selection:bg-primary selection:text-black text-left">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center opacity-10 gap-24 py-60">
                        ...
                        <p className="text-4xl md:text-[10rem] font-bold italic text-gray-500 uppercase tracking-widest max-w-[140rem]">Interrogate the Absolute Material Core to Materialize Knowledge DNA.</p>
                    </div>
                 )}
              </CardContent>
              ...
              <div className="p-16 border-t-8 border-white/5 mt-auto flex justify-between items-center opacity-35 text-[20px] font-black uppercase tracking-[8em] italic">
                 <span>ABSOLUTE_NEX_v75_AL_GHAZALI_ROOT</span>
