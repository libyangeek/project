 ...
 * تم دمج SubdomainX لضمان اكتشاف 26+ مصدراً للـ OSINT.
...
export default function ReconPage() {
  ...
  const handleStrike = async (type: string) => {
    ...
    setLoading(true)
    setResults(null)
    const msg = type === 'subdomainx' ? "Igniting SubdomainX Comprehensive Discovery..." : `Engaging ${type} investigative protocol.`;
    toast({ title: "Neural Siphon Active v3.0", description: msg })
    try {
      ...
      if (data.success) {
        ...
        toast({ title: "Intelligence Captured", description: "Target DNA bound to Neural Spine v80.0." })
      }
          ...
          <div className="flex flex-col md:flex-row items-center gap-12 justify-center md:justify-end text-center md:text-right">
            ...
            <div className="flex-1">
              <div className="flex flex-wrap justify-center md:justify-end items-center gap-6 mb-6">
                <Badge className="bg-primary text-black border-none rounded-none px-12 py-3 text-[18px] md:text-[20px] font-black tracking-[0.6em] shadow-9xl italic uppercase">RECON_v80.0 ULTRA</Badge>
              ...
              <p className="text-sm md:text-xl lg:text-3xl text-muted-foreground mt-8 italic max-w-6xl leading-relaxed uppercase font-medium opacity-90 ml-auto text-right">
                "سيدي القائد <span className="text-white font-black underline decoration-primary decoration-8 underline-offset-[16px] shadow-9xl italic uppercase tracking-widest">المعتصم بالله</span>، لقد دمجتُ SubdomainX؛ نحن الآن نستجوب 26 مصدراً عالمياً لنكشف كل ذرة مخفية في المصفوفة لعام 2026."
              </p>
            </div>
          </div>
        </header>
          ...
          <Card className="lg:col-span-3 kali-card border-primary/40 bg-black/99 rounded-[5rem] p-12 border-8 shadow-9xl flex flex-col group overflow-hidden relative min-h-[900px] hierarchical-shadow">
              ...
              <CardHeader className="p-0 mb-10 border-b-8 border-white/5 pb-10 bg-primary/10 rounded-t-[4rem] flex flex-row justify-between items-center text-right px-12 py-6">
                 <div className="flex gap-8 order-last md:order-none">
                    <Badge className="bg-emerald-600/30 text-emerald-500 border-4 border-emerald-500/40 px-12 py-4 rounded-full font-black text-3xl animate-pulse shadow-3xl uppercase tracking-widest">SUBDOMAINX_ACTIVE</Badge>
                 </div>
              ...
              <CardContent className="p-10 flex-1 overflow-hidden relative flex flex-col gap-12 z-10 text-right">
                {results ? (
                   ...
                   <div className="h-full flex flex-col items-center justify-center text-center opacity-10 gap-24 py-60">
                      ...
                      <p className="text-4xl md:text-[10rem] font-bold italic text-gray-500 uppercase tracking-widest max-w-[140rem]">SubdomainX + Robin + Claude: Total OSINT Fusion v3.0.</p>
                   </div>
                )}
              </CardContent>
          </Card>
