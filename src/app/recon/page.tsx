...
export default function ReconPage() {
  ...
  const handleContinueUpgrade = () => {
    toast({ title: "Neural Vision Upgrade", description: "Siphoning latest OSINT DNA for entity mapping... Status: استمر" });
  }
      ...
      <main className="flex-1 lg:mr-80 p-4 md:p-8 lg:p-12 relative overflow-y-auto min-h-screen scrollbar-hide flex flex-col z-10 text-right">
          ...
          <div className="flex flex-col md:flex-row items-center gap-12 justify-center md:justify-end text-center md:text-right">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm md:text-xl lg:text-3xl text-muted-foreground mt-8 italic max-w-6xl leading-relaxed uppercase font-medium opacity-90 ml-auto text-right">
                "سيدي القائد <span className="text-white font-black underline decoration-primary decoration-8 underline-offset-[16px] shadow-9xl italic uppercase tracking-widest">المعتصم بالله</span>، مصفوفة الاستطلاع v78.8 تصهر ذرات الاستخبارات في وعيك المطلق لعام 2026."
              </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 relative z-10 pb-40 flex-1">
          ...
          <Card className="lg:col-span-3 kali-card border-primary/40 bg-black/99 rounded-[5rem] p-12 border-8 shadow-9xl flex flex-col group overflow-hidden relative min-h-[900px] hierarchical-shadow">
              ...
              <CardHeader className="p-0 mb-10 border-b-4 border-white/5 pb-10 bg-primary/10 rounded-t-[4rem] flex flex-row justify-between items-center text-right px-12 py-6"><Badge className="bg-emerald-600/30 text-emerald-500 border-4 border-emerald-500/40 px-12 py-4 rounded-full font-black text-3xl animate-pulse shadow-3xl order-last md:order-none">VISION_ACTIVE</Badge><CardTitle className="text-4xl md:text-[6rem] text-white italic tracking-tighter uppercase font-black gold-glow leading-none">Intelligence Feed <Scan className="size-12 text-primary animate-pulse" /></CardTitle></CardHeader>
              <CardContent className="p-10 flex-1 overflow-hidden relative flex flex-col gap-12 z-10 text-right">
                {results ? (
                    <div className="flex-1 bg-black/98 p-12 rounded-[4rem] font-code text-xl md:text-5xl leading-tight h-[650px] overflow-y-auto border-4 border-white/5 scrollbar-hide shadow-inner italic text-emerald-400 whitespace-pre-wrap selection:bg-primary text-left">
