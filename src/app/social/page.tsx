...
export default function SocialPredatorPage() {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      } else if (activeTab === 'xlogger') {
          toast({ title: "XLogger Node Active", description: "Identity Siphon listening on port 8888." });
      } else if (activeTab === 'seeker') {
          toast({ title: "Seeker GPS Node Active", description: "Awaiting coordinate ping from target mesh..." });
      }
    } catch (err) {
