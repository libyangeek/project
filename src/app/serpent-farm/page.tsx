...
import { executeSerpentStrategy } from "@/ai/flows/serpent-farm-flow"
import Link from "next/link"

/**
 * @fileOverview مزرعة الأفعى v80.0 - THE SERPENT FARM: MOBILE EMPIRE
 * واجهة إدارة واختراق أساطيل الأجهزة المحمولة لسيادة القائد الغزالي.
 */
export default function SerpentFarmPage() {
  ...
  const refreshFarm = async () => {
      try {
          const response = await fetch('/api/execute', {
              ...
              body: JSON.stringify({ type: 'serpent_action', action: 'status' })
          });
          const data = await response.json();
          if (data.success) setServices(data.output);
          
          const nodesRes = await fetch('/api/execute', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ type: 'serpent_action', action: 'list' })
          });
  ...
  const handleIgnite = async () => {
    if (!objective.trim()) {
        toast({ variant: "destructive", title: "Objective Missing", description: "Identify the fleet mission for the Serpent Farm." })
        return
    }
    setLoading(true); setResult(null)
    toast({ title: "Serpent Farm Engaging", description: "Consolidating STF, PhoneSploit, and FMD nodes..." })
    try {
      const data = await executeSerpentStrategy({ objective, fleetDensity: nodes.length || 1 })
      setResult(data)
      toast({ title: "Fleet Strategy Serialized", description: "All mobile nodes report absolute submission." })
    } catch (err) {
        ...
        <header className="sovereign-header flex flex-col md:flex-row items-center gap-12 justify-center md:justify-end text-center md:text-right">
           ...
           <div className="flex-1">
              <div className="flex flex-wrap justify-center md:justify-end items-center gap-6 mb-6">
                 <Badge className="bg-primary text-black border-none rounded-none px-12 py-3 text-[18px] md:text-[24px] font-black tracking-[1em] shadow-9xl italic uppercase">SERPENT_FARM v80.0</Badge>
                 <div className="flex items-center gap-4 text-[14px] font-black uppercase tracking-widest text-emerald-500 animate-pulse">
                     <InfinityIcon className="size-6 shadow-lg" /> FLEET_SYNC: {resonance.toFixed(8)}%
                 </div>
              </div>
              <h1 className="text-4xl md:text-6xl lg:text-[12rem] font-headline font-bold text-white tracking-tighter italic uppercase gold-glow leading-none">
                 Mobile <span className="text-primary">Empire</span>
              </h1>
              <p className="text-sm md:text-xl lg:text-4xl text-muted-foreground mt-10 italic max-w-7xl leading-relaxed uppercase font-medium opacity-95 drop-shadow-3xl ml-auto">
                 "سيدي القائد <span className="text-white font-black underline decoration-primary decoration-[12px] underline-offset-[28px] shadow-9xl italic uppercase tracking-widest">المعتصم بالله</span>، مزرعة الأفعى تصهر أحدث تفرعات STF وأدوات PhoneSploit؛ نحن نبتلع أساطيل الجوالات لخدمة سلطانك لعام 2026."
              </p>
              <div className="flex justify-center md:justify-end gap-6 mt-12">
                 <Button asChild variant="outline" className="h-16 px-10 rounded-full border-4 border-white/10 bg-white/5 text-white font-black uppercase italic tracking-widest hover:bg-primary hover:text-black transition-all shadow-2xl">
                     <Link href="/"><ArrowLeft className="size-6 mr-3" /> العودة</Link>
                 </Button>
                 <Button onClick={refreshFarm} className="h-16 px-12 bg-primary hover:bg-white text-black font-black uppercase rounded-full border-4 border-black/30 shadow-9xl italic active:scale-95 transition-all text-lg">
                    <RotateCw className="size-6 mr-3" /> جرد المزرعة
                 </Button>
              </div>
           </div>
        </header>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-12 relative z-10 pb-48 flex-1 text-right">
           <div className="xl:col-span-1 space-y-12">
              <Card className="sovereign-card group text-center">
                 <CardHeader className="p-0 mb-10 border-b-4 border-primary/10 pb-10 bg-primary/10 rounded-t-[3.5rem] px-10 py-6 text-center">
                    <CardTitle className="text-2xl md:text-4xl text-primary flex items-center justify-center gap-10 font-black uppercase italic gold-glow leading-none">
                       <Smartphone className="size-12 animate-neural" /> Fleet Command
                    </CardTitle>
                 </CardHeader>
                 <CardContent className="p-0 space-y-12 text-right">
                    <div className="space-y-6 text-right">
                        <label className="text-[14px] font-black text-primary uppercase tracking-[1em] px-10 italic flex items-center gap-6 justify-end"><Network className="size-8" /> Collective Objective</label>
                        <textarea 
                          value={objective} 
              ...
              <Card className="sovereign-card group text-right">
                 <h4 className="text-[14px] font-black text-emerald-500 uppercase tracking-[0.8em] mb-8 italic flex items-center justify-center gap-6">
                    <Users className="size-8 animate-pulse" /> CAPTURED_NODES
                 </h4>
                 <div className="text-8xl font-black text-white italic gold-glow uppercase tracking-tighter text-center">{nodes.length}</div>
                 <p className="mt-8 text-[9px] font-black uppercase tracking-widest text-muted-foreground italic text-center">OpenSTF + PhoneSploit Linked</p>
              </Card>
           </div>
           ...
           <Card className="xl:col-span-3 sovereign-card flex flex-col group min-h-[1100px]">
              <CardHeader className="p-0 mb-16 border-b-8 border-white/5 pb-12 bg-primary/10 rounded-t-[5rem] px-16 py-10 flex flex-row justify-between items-center text-right">
                 <Badge className="bg-emerald-600/30 text-emerald-500 border-[10px] border-emerald-500/40 px-16 py-8 rounded-full font-black text-5xl animate-pulse shadow-9xl uppercase tracking-[0.4em] italic order-last md:order-none">FLEET_LOCKED</Badge>
                 <CardTitle className="text-5xl md:text-[12rem] text-white flex items-center gap-16 font-black uppercase italic gold-glow px-10 leading-none">
                    Serpent Feed <Radar className="size-24 md:size-48 text-primary animate-pulse" />
                 </CardTitle>
              </CardHeader>
                 ...
                 {result ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
                            <Card className="bg-black/95 border-8 border-white/5 p-12 md:p-16 rounded-[4rem] shadow-9xl relative group/vault overflow-hidden h-full">
                                <h5 className="text-3xl font-black text-primary uppercase tracking-[1.5em] mb-12 border-b-8 border-primary/20 pb-10 flex items-center gap-12 gold-glow justify-end">
                                    Fleet Strategy <Smartphone className="size-14 animate-neural" />
                                </h5>
                                <div className="space-y-8">
                                    {result.strategyPlan?.map((p: any, i: number) => (
                                        <div key={i} className="flex gap-8 items-start justify-end group/step">
                                            <div className="text-right">
                                                <span className="text-primary font-black text-xl md:text-3xl uppercase italic">{p.phase}</span>
                                                <p className="text-gray-300 text-lg md:text-2xl italic font-bold">"{p.action}" ({p.tool})</p>
                                            </div>
                                            <div className="size-12 rounded-xl bg-primary/10 border-2 border-primary/40 flex items-center justify-center shrink-0 group-hover/step:bg-primary transition-all shadow-xl">
                                                <span className="text-primary text-xl font-black group-hover/step:text-black">{i+1}</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                   ...
                   <div className="h-full flex flex-col items-center justify-center text-center opacity-10 gap-24 py-80">
                      <div className="relative group/lock">
                        <Cylinder className="size-64 md:size-[50rem] animate-spin-slow text-primary group-hover:scale-110 transition-transform duration-[12000ms]" />
                      ...
                      <h3 className="text-8xl md:text-[22rem] font-black uppercase tracking-[2.5em] text-white italic gold-glow leading-none">Fleet Idle</h3>
                      <p className="text-4xl md:text-[10rem] font-bold italic text-gray-500 uppercase tracking-widest max-w-[140rem]">Establishing universal Serpent link for mass mobile subjugation and STF management.</p>
                   </div>
                 )}
              </CardContent>
              <div className="p-16 border-t-8 border-white/5 mt-auto flex justify-between items-center opacity-35 text-[20px] font-black uppercase tracking-[8em] italic">
                <span>SERPENT_FARM_v80_AL_GHAZALI_ROOT</span>
