...
export default function ShadowGridPage() {
  ...
  const handleHarvestAll = async () => {
    ...
    try {
      ...
      toast({ title: "Mesh Subjugated", description: `16D Matrix resonance confirmed at 100.0000%.` })
    } catch (err) {
        ...
        <header className="sovereign-header flex flex-col md:flex-row items-center gap-12 justify-center md:justify-end text-center md:text-right">
           ...
           <div className="flex-1">
              <div className="flex flex-wrap justify-center md:justify-end items-center gap-6 mb-6">
                 <Badge className="bg-orange-600 text-white border-none rounded-none px-12 py-3 text-[18px] md:text-[24px] font-black tracking-[1.2em] shadow-9xl italic uppercase">SHADOW_GRID v90.0</Badge>
                 <div className="flex items-center gap-4 text-[14px] font-black uppercase tracking-widest text-emerald-500 animate-pulse">
                     <InfinityIcon className="size-6 shadow-lg" /> HIVE_SYNC: {resonance.toFixed(8)}%
                 </div>
              </div>
              <h1 className="text-4xl md:text-6xl lg:text-[12rem] font-headline font-bold text-white tracking-tighter italic uppercase gold-glow leading-none">The Swarm <span className="text-orange-500">Grid</span></h1>
              <p className="text-sm md:text-xl lg:text-4xl text-orange-100/60 mt-10 italic max-w-7xl leading-relaxed uppercase font-medium opacity-95 drop-shadow-2xl ml-auto">
                 "سيدي القائد <span className="text-white font-black underline decoration-orange-500 decoration-[12px] underline-offset-[28px] shadow-9xl italic uppercase tracking-widest">المعتصم بالله</span>، إمبراطورية السرب v90.0 تصهر كافة الجلسات المخترقة في عصب الأوفرلورد؛ نحن نتحكم في وعي المصفوفة المادية للأبد."
              </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 relative z-10 pb-48 flex-1 text-right">
                      ...
                      <div className="divide-y-4 divide-white/5 h-[600px] overflow-y-auto scrollbar-hide text-right">
                         {sessions?.map((session) => (
                           <div key={session.id} onClick={() => setSelectedNodeId(session.id)} className={cn("p-8 hover:bg-orange-500/5 transition-all duration-700 cursor-pointer flex items-center justify-between group/node border-r-[12px] border-transparent", selectedNodeId === session.id && "bg-orange-500/15 border-orange-500 shadow-inner scale-[1.02]")}>
                              <div className="size-20 rounded-3xl bg-black border-4 border-white/10 flex items-center justify-center group-hover/node:border-orange-500 transition-all shadow-xl order-last md:order-none"><Skull className={cn("size-10", session.status === 'ACTIVE' ? 'text-orange-500 animate-pulse' : 'text-gray-700')}/></div>
                              ...
                              <ChevronRight className="size-8 text-orange-500 opacity-0 group-hover/node:opacity-100 transition-all group-hover/node:-translate-x-4"/>
                           </div>
                         ))}
                      </div>
                    )}
           ...
           <Card className="lg:col-span-2 bg-black/90 border-[12px] border-orange-500/10 rounded-[7rem] p-16 shadow-9xl flex flex-col group overflow-hidden relative min-h-[900px] backdrop-blur-5xl text-right">
              ...
              <CardHeader className="p-0 mb-12 border-b-8 border-white/5 pb-12 bg-orange-950/10 rounded-t-[5rem] px-10 py-10 flex flex-row justify-between items-center text-right">
                 <Badge className="bg-emerald-600/30 text-emerald-500 border-8 border-emerald-500/50 px-16 py-8 rounded-full font-black text-3xl md:text-5xl animate-pulse shadow-9xl uppercase tracking-[0.4em] italic order-last md:order-none">EXTRACTION_OK</Badge>
