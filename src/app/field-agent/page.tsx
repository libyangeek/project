...
export default function FieldAgentPage() {
  ...
  const handleSovereignAction = async (mode: 'file_fix' | 'integrity_sync') => {
    setLoading(true)
    try {
      const result = await executeFieldDevelopment({ 
        userPrompt: input || "Fix target DNA node.",
        projectPath: currentPath,
  ...
  const handleGeneticInjection = async () => {
    ...
    try {
        ...
        const data = await response.json()
        if (data.success) toast({ title: "Genetic Fusion Success", description: "Hardware DNA rewritten." })
    } finally {
        setLoading(false)
    }
  }
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-12">
                 <Card className="kali-card border-primary/20 bg-black/95 rounded-[4rem] p-10 shadow-9xl flex flex-col h-[750px] border-4">
                    ...
                    <Textarea 
                        ...
                        className="flex-1 bg-black/99 p-10 rounded-[3rem] font-code text-xl text-emerald-400 leading-relaxed italic border-8 border-white/5 shadow-inner whitespace-pre selection:bg-primary resize-none scrollbar-hide"
                    />
                 </Card>
           ...
           <div className="absolute bottom-0 left-0 right-0 p-10 bg-black/98 backdrop-blur-5xl border-t-[12px] border-primary/60 z-30 shadow-[0_-80px_250px_rgba(0,0,0,1)]">
              <div className="max-w-7xl mx-auto relative group">
                 ...
                 <Button onClick={() => handleSovereignAction('file_fix')} className="absolute right-4 top-1/2 -translate-y-1/2 size-24 bg-primary hover:bg-white text-black rounded-full shadow-9xl border-[10px] border-black/30 group">
                    {loading ? <Loader2 className="size-12 animate-spin" /> : <Send className="size-12 group-hover:translate-x-3 transition-transform" />}
                 </Button>
              </div>
           </div>
        </div>
