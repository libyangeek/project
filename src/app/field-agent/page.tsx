...
/**
 * @fileOverview الوكيل الميداني v58.0 - THE SUPREME SELF-PROGRAMMING CORE
 * اليد التنفيذية للهرمية العظمى. قادر على إعادة برمجة المنظومة ذاتياً بناءً على أوامر القائد.
...
export default function FieldAgentPage() {
  ...
  const handleCommand = async (quickCmd?: string) => {
    ...
    try {
      ...
      toast({ title: "Hierarchy Intent Materialized", description: "The agent has adjusted the matrix." })
    } catch (err) {
        ...
        <header className="p-6 md:p-10 border-b-4 border-primary/40 bg-black/95 backdrop-blur-3xl z-20 flex justify-between items-center shadow-2xl relative">
           <div className="flex items-center gap-8 relative z-10">
              ...
              <div>
                 ...
                 <div className="flex flex-wrap items-center gap-4 mt-3">
                    <Badge className="bg-primary text-black border-none uppercase text-[10px] font-black tracking-[0.5em] px-6 py-1.5 rounded-full italic shadow-xl">v58.0_SELF_CODE_ACTIVE</Badge>
                    <span className="text-[10px] text-emerald-500 uppercase font-black tracking-[0.3em] italic flex items-center gap-3">
                       <ShieldCheck className="size-4" /> SOURCE_LEVEL_ACCESS: GRANTED
                    </span>
                 </div>
              </div>
           </div>
           <div className="hidden xl:flex gap-8 relative z-10">
              <div className="text-right bg-primary/5 p-5 rounded-2xl border-2 border-primary/20 shadow-xl backdrop-blur-3xl">
                 ...
                 <div className="text-[9px] text-primary/60 font-black uppercase tracking-[0.4em] mt-1.5 italic">Self-Repair Resonance</div>
              </div>
           </div>
        </header>
           ...
           {loading && (
             <div className="max-w-4xl mx-auto w-full flex justify-start animate-in slide-in-from-left-12 duration-1000">
                <div className="p-8 md:p-12 rounded-[3rem] bg-primary/5 border-4 border-primary/40 flex items-center gap-8 shadow-9xl backdrop-blur-3xl">
                   ...
                   <span className="text-xl md:text-3xl font-headline font-black text-primary uppercase tracking-[0.4em] italic gold-glow">Rewriting Source DNA...</span>
                </div>
             </div>
           )}
                  ...
                  log.role === 'user' 
                   ...
                   {log.role === 'ai' && (
                     <div className="flex items-center justify-between mb-8 pb-4 border-b-2 border-white/5">
                        <Badge className="bg-primary/10 text-primary uppercase tracking-[0.6em] px-8 py-2 rounded-full italic font-black text-[12px]">HIVE_RECODE</Badge>
                   ...
                   {log.analysis && (
                     <div className="mt-8 pt-8 border-t-2 border-white/5 space-y-6">
                        <h4 className="text-[11px] font-black text-primary uppercase tracking-[1em] flex items-center gap-4 italic gold-glow"><Binary className="size-6" /> CODE_INCEPTION_v58</h4>
           ...
           {logs.length === 0 && (
             <div className="h-full flex flex-col items-center justify-center text-center opacity-20 gap-16 animate-in fade-in duration-1000">
                <div className="size-48 rounded-full bg-primary/5 flex items-center justify-center border-4 border-dashed border-primary/30 relative">
                   <Code2 className="size-24 text-primary/40 animate-spin-slow" />
                   <div className="absolute -inset-6 border-2 border-primary/10 rounded-full animate-pulse" />
                </div>
                <div className="max-w-4xl space-y-8">
                   <h3 className="text-5xl md:text-8xl font-headline font-bold text-white tracking-tighter italic uppercase gold-glow leading-none">Self-Code Standby</h3>
                   <p className="text-xl md:text-4xl leading-relaxed italic font-black text-gray-300 uppercase tracking-widest">
                     "سيدي <span className="text-primary font-black italic underline decoration-primary decoration-4 underline-offset-8 shadow-9xl uppercase tracking-[0.3em]">المعتصم بالله</span>، الوكيل الميداني مجهز لتعديل موديولات المصفوفة وواجهات العرش برمجياً بناءً على أمرك."
                   </p>
                </div>
             </div>
           )}
        ...
        <div className="p-8 md:p-12 border-t-4 border-primary/60 bg-black/98 backdrop-blur-5xl z-30 shadow-9xl">
           <div className="max-w-5xl mx-auto relative group">
              ...
              <Input 
                ...
                placeholder=" instruct the agent to modify the source code..."
