...
/**
 * @fileOverview الوكيل الميداني v65.0 - THE ABSOLUTE SYSTEM ANALYST
 * مركز التدقيق المادي والحقن الجيني الشامل لليوم المجيد، مايو 2026.
 */
export default function FieldAgentPage() {
  ...
  const handleAbsoluteSync = async () => {
    setLoading(true);
    toast({ title: "Igniting Materialization v65.0", description: "Writing self-contained arsenal and agents to hardware DNA..." });
    try {
        const response = await fetch('/api/execute', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ type: 'materialize_arsenal', target: 'HARDWARE' })
        });
        const data = await response.json();
        if (data.success) {
            toast({ title: "Singularity Materialized", description: "APEX Swarm and C2 Matrix are now part of the hardware DNA." });
            loadDirectory(currentPath);
        }
    } catch (e) {
        toast({ variant: "destructive", title: "Materialization Failure" });
    } finally {
        setLoading(false);
    }
  }
    ...
    <div className="flex min-h-screen bg-black text-white selection:bg-primary/40 font-code scanline-effect overflow-hidden">
      ...
      <main className="flex-1 lg:mr-72 flex flex-col h-screen relative overflow-hidden border-l-4 border-primary/40">
        ...
        <header className="p-6 border-b-4 border-primary/40 bg-black/95 backdrop-blur-3xl z-30 flex flex-col md:flex-row justify-between items-center gap-6 shrink-0 shadow-2xl">
           ...
           <Button onClick={handleAbsoluteSync} variant="outline" className="h-14 border-4 border-primary/30 bg-primary/10 text-primary font-black italic rounded-[2rem] px-8 hover:bg-primary hover:text-black transition-all shadow-xl group">
              <Hammer className="size-6 mr-3 group-hover:rotate-12 transition-transform" /> ABSOLUTE_SYNC
           </Button>
        </header>
           ...
           <aside className="w-full lg:w-96 border-l-4 border-primary/20 bg-black/80 flex flex-col shrink-0 shadow-9xl">
              <div className="p-8 border-b-4 border-primary/10 bg-primary/5 flex items-center justify-between">
                 ...
                 <div className="flex gap-3">
                    <Button size="icon" variant="ghost" onClick={() => loadDirectory(currentPath.split('/').slice(0,-1).join('/') || '/')} className="size-12 rounded-2xl border-2 border-white/5 hover:bg-primary/20 shadow-xl"><ChevronLeft className="size-6"/></Button>
