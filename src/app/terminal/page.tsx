...
export default function TerminalPage() {
  ...
  return (
    <div className="flex min-h-screen bg-black text-white selection:bg-red-600/30">
      <SidebarNav />
      <main className="flex-1 ml-64 flex flex-col h-screen overflow-hidden bg-[radial-gradient(circle_at_center,rgba(239,68,68,0.2),transparent)]">
        {/* Predator Terminal Header */}
        <header className="p-12 border-b-2 border-red-600/40 flex items-center justify-between bg-red-950/60 backdrop-blur-3xl z-20 border-2 shadow-2xl">
        ...
        <div className="flex-1 flex min-h-0 relative">
          <div className="flex-1 flex flex-col min-w-0 bg-black/40 scan-line">
                    ...
                    <div className={cn(
                      ...
                      {msg.chain && (
                        <div className="space-y-12 mt-16 p-14 bg-black/90 rounded-[4.5rem] border-2 border-red-500/40 shadow-inner">
                           <h4 className="text-[16px] font-bold text-red-500 uppercase tracking-[1.2em] mb-12 flex items-center gap-10 italic border-b border-red-600/20 pb-8">
                             <Network className="size-10" /> Global Kill-Chain Sequence
                           </h4>
                                    ...
                                    <div className="flex-1">
                                      <div className="flex justify-between items-center mb-6">
                                          ...
                                          <div className="flex gap-6">
                                            {step.nodeId && <Badge className="bg-red-600 text-white text-[11px] px-6 py-2 rounded-2xl border-2 border-red-400 shadow-2xl">NODE: {step.nodeId}</Badge>}
                                            <Badge className="bg-accent/20 text-accent text-[11px] px-6 py-2 border-2 border-accent/50 uppercase tracking-[0.3em] font-bold italic shadow-2xl">ULT_MOD_{step.module}</Badge>
                                          </div>
                                      </div>
                        ...
                        <div className="mt-16 space-y-12">
                           <div className="flex items-center justify-between text-red-500 px-6">
                              <div className="flex items-center gap-8">
                                ...
                                <span className="text-2xl font-bold uppercase tracking-[1em] italic drop-shadow-[0_0_20px_red]">God-Tier Weaponry Synthesized</span>
                              </div>
                              <Button variant="ghost" size="icon" className="size-24 hover:bg-red-600/20 rounded-[2.5rem] shadow-2xl transition-all duration-1000 bg-white/5 border-2 border-transparent hover:border-red-600/50" onClick={() => {
                                ...
                                toast({ title: "Sovereign Weaponry Extracted" });
                              }}>
                      ...
                      {msg.role === "assistant" && (
                        <div className="mt-20 pt-16 border-t-2 border-white/10 flex gap-16">
                           <Button className="terminal-button h-28 px-24 rounded-[3.5rem] font-bold uppercase tracking-[1em] text-[15px] shadow-[0_40px_100px_rgba(220,38,38,0.7)] group transition-all duration-1000 border-2 border-red-400/50 active:scale-95 italic">
                              <Zap className="size-10 mr-8 group-hover:scale-125 transition-transform" /> Execute Kill-Chain
                           </Button>
                           <Button variant="outline" className="h-28 px-24 rounded-[3.5rem] border-2 border-white/20 bg-white/5 text-[15px] uppercase font-bold tracking-[1em] hover:bg-red-600/20 hover:border-red-600/70 transition-all duration-1000 italic shadow-2xl">
                              <ShieldX className="size-10 mr-8 text-red-600" /> Neural Impact Test
                           </Button>
                        </div>
                      )}
                    </div>
            ...
            <div className="p-24 border-t-2 border-red-600/60 bg-red-950/50 backdrop-blur-3xl shadow-[0_-80px_200px_rgba(0,0,0,1)] relative z-30">
              <div className="max-w-[1500px] mx-auto">
                <form onSubmit={handleSend} className="relative group/form">
                  ...
                  <Input
                    ...
                    placeholder="Dictate Supreme Sovereign Intent..."
