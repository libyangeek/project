...
/**
 * @fileOverview المحطة الأبدية v78.8 - THE SUPREME OVERMIND SHELL
...
export default function TerminalPage() {
  ...
  const handleContinueUpgrade = () => {
    toast({ title: "Shell Expansion Triggered", description: "Injecting next-gen kernel modules for direct material command... Status: استمر" });
  }
      ...
      <main className="flex-1 lg:mr-80 flex flex-col h-screen overflow-hidden bg-black relative border-l-4 border-primary/40 text-right">
        <header className="p-6 md:p-8 border-b-4 border-primary/60 bg-black/95 backdrop-blur-3xl z-20 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl">
          <div className="flex gap-4">
             <Button asChild variant="outline" className="h-14 md:h-16 px-6 md:px-10 rounded-full border-4 border-white/10 bg-white/5 text-white font-black uppercase italic tracking-widest hover:bg-primary hover:text-black transition-all shadow-2xl">
                <Link href="/"><ArrowLeft className="size-6 mr-3" /> العودة</Link>
             </Button>
             <Button onClick={handleContinueUpgrade} className="h-14 md:h-16 px-8 md:px-12 bg-primary hover:bg-white text-black font-black uppercase rounded-full border-4 border-black/30 shadow-9xl italic transition-all active:scale-90">
          ...
          <div className="flex items-center gap-6 md:gap-8">
            <div className="text-right">
              <h2 className="text-3xl md:text-6xl font-headline font-bold text-white uppercase italic tracking-tighter gold-glow leading-none">Supreme Shell <span className="text-primary">v78.8</span></h2>
              <div className="flex items-center gap-6 text-[10px] md:text-[11px] text-primary font-black uppercase tracking-[0.5em] mt-3 italic justify-end">
                <span>ULTRA_OVERMIND_LOCKED</span>
        ...
        <div className="flex-1 flex min-h-0 flex-col relative z-10">
          <ScrollArea className="flex-1 p-6 md:p-12 font-mono scrollbar-hide text-right">
            <div className="space-y-12 pb-32">
              {messages.map((msg, i) => (
                <div key={i} className={cn(
                  ...
                  <div className={cn(
                    ...
                    {msg.content}
                    {msg.role === "user" && <span className="text-primary ml-4 md:ml-8 select-none font-bold">❯❯❯</span>}
                  </div>
                  {msg.role === 'assistant' && (
                    <div className="absolute top-0 left-0 p-8 md:p-12 opacity-5 pointer-events-none group-hover:opacity-10 transition-opacity">
              ...
              {isLoading && (
                <div className="flex items-center gap-8 text-primary animate-pulse mr-8 md:mr-16 py-10 justify-end">
          ...
          <div className="p-6 md:p-12 bg-black/99 border-t-8 md:border-t-[12px] border-primary/60 shadow-[0_-50px_200px_rgba(0,0,0,1)] z-30">
            <form onSubmit={executeCommand} className="max-w-7xl mx-auto relative flex items-center gap-6 md:gap-12 bg-white/5 rounded-[3rem] md:rounded-[5rem] px-8 md:px-16 focus-within:border-primary transition-all duration-1000 border-4 border-white/10 shadow-inner">
              ...
              <Input
                ...
                className="flex-1 bg-transparent border-none focus-visible:ring-0 text-white font-mono text-2xl md:text-[5.5rem] h-32 md:h-48 placeholder:text-gray-900 italic font-black selection:bg-primary text-right"
                disabled={isLoading}
              />
            ...
            <div className="flex justify-center gap-12 md:gap-24 mt-8 opacity-30 text-[10px] md:text-[14px] font-black uppercase tracking-[3em] md:tracking-[6em] italic text-white overflow-hidden whitespace-nowrap">
                ...
                <span className="flex items-center gap-4 text-emerald-500"><InfinityIcon className="size-5 animate-pulse" /> SPINE_SYNC_v78.8</span>
            </div>
          </div>
        </div>
      </main>
