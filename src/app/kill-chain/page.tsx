...
/**
 * @fileOverview سلسلة الإبادة الكونية v53.5 - AUTONOMOUS EXECUTION
 * الواجهة الأسمى التي تربط الاستطلاع بالعراف بالتنفيذ المستقل.
 */
export default function KillChainPage() {
  ...
  const startKillChain = async () => {
    ...
    try {
      toast({ title: "Autonomous Strike Initiated", description: "Alpha-Core is orchestrating the kill-chain." })
      
      // محاكاة الخطوات المرئية لتعزيز التجربة
      for(let i=1; i<=4; i++) {
        setActiveStep(i)
      ...
      setChainResult(data)
      toast({ title: "Subjugation Stabilized", description: "Target matrix absorbed into the hierarchy." })
    } catch (err) {
  ...
  const steps = [
    { id: 1, label: "Trace-Labs Recon", icon: Eye, desc: "Autonomous OSINT Extraction" },
    { id: 2, label: "Oracle Search", icon: Radar, desc: "Real-time KEV Mapping" },
    { id: 3, label: "DNA Synthesis", icon: Flame, desc: "Exploit Forge Generation" },
    { id: 4, label: "Hive Injection", icon: Ghost, desc: "Autonomous System Execution" }
  ]
      ...
      <main className="flex-1 lg:mr-80 p-4 md:p-8 lg:p-12 relative overflow-y-auto min-h-screen scrollbar-hide flex flex-col z-10">
        ...
        <header className="mb-12 relative z-10 animate-in fade-in slide-in-from-top-6 duration-1000">
          <div className="flex flex-col md:flex-row items-center gap-8">
            <div className="size-24 md:size-32 bg-black border-4 border-primary flex items-center justify-center shadow-[0_0_100px_rgba(212,175,55,0.6)] relative group shrink-0 rounded-3xl transition-all duration-1000 hierarchical-shadow">
            ...
            <div className="text-center md:text-right flex-1">
               <div className="flex flex-wrap justify-center md:justify-start items-center gap-4 mb-4">
                  <Badge className="bg-primary text-black border-none rounded-none px-8 py-2 text-[14px] md:text-[16px] font-black tracking-[0.5em] shadow-2xl italic uppercase">KILL_CHAIN v53.5</Badge>
                  <div className="flex items-center gap-3 text-[11px] font-black uppercase tracking-widest text-emerald-500 animate-pulse">
                      <ShieldCheck className="size-5 shadow-lg" /> AUTONOMOUS_SYNC: LOCKED
                  </div>
               </div>
               <h1 className="text-4xl md:text-6xl lg:text-8xl font-headline font-bold text-white tracking-tighter italic uppercase gold-glow leading-none">Universal <span className="text-primary">Subjugation</span></h1>
               <p className="text-sm md:text-xl lg:text-2xl text-muted-foreground mt-4 italic max-w-5xl leading-relaxed uppercase font-medium opacity-80">
                  "سيدي الغزالي، سلسلة الإبادة تعمل الآن باستقلالية كاملة؛ المُعِزّ يحلل ويخلق وينفذ بضمان فتك 100% لعام 2026."
               </p>
            </div>
          </div>
        </header>
              ...
              <Card className="kali-card border-primary/30 bg-black/98 rounded-3xl p-8 border-2 shadow-2xl group overflow-hidden hierarchical-shadow">
                 ...
                 <CardContent className="p-0 space-y-8">
                    <div className="space-y-4">
                        <label className="text-[10px] font-black text-primary/60 uppercase tracking-[0.6em] px-4 italic flex items-center gap-3"><Globe className="size-4" /> Strike Coordinate</label>
                        <Input 
                          value={target}
              ...
              <div className="space-y-4">
                 {steps.map((s) => (
                   <div 
                     key={s.id} 
                     className={cn(
                       "p-6 rounded-2xl border-2 transition-all duration-700 flex items-center gap-6 relative overflow-hidden",
                       activeStep >= s.id ? "bg-primary/10 border-primary shadow-lg scale-105" : "bg-white/5 border-white/5 opacity-40 grayscale"
                     )}
                   >
                      <div className={cn("size-12 rounded-xl flex items-center justify-center border-2 transition-all", activeStep >= s.id ? "bg-primary border-black" : "bg-black border-white/10")}>
           ...
           <Card className="xl:col-span-3 kali-card border-primary/40 bg-black/99 rounded-[3rem] p-8 border-2 shadow-9xl flex flex-col group overflow-hidden relative min-h-[850px] hierarchical-shadow">
              ...
              <CardHeader className="p-0 mb-8 border-b-2 border-white/5 pb-8 bg-primary/5 rounded-t-3xl px-8 py-4 flex flex-row justify-between items-center">
                 <CardTitle className="text-3xl md:text-7xl text-white italic uppercase font-black gold-glow flex items-center gap-8 px-4 leading-none">
                    <Binary className="size-12 md:size-20 text-primary animate-pulse" /> Strike Feed
                 </CardTitle>
                 {chainResult && (
                   <Badge className="bg-emerald-600/20 text-emerald-500 border-2 border-emerald-500/30 px-8 py-2 rounded-full font-black text-xl animate-pulse tracking-[0.2em] uppercase italic shadow-lg">ACQUISITION_ACTIVE</Badge>
                 )}
              </CardHeader>
                 ...
                 {chainResult ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                         <Card className="bg-black/80 border-2 border-white/5 p-8 rounded-3xl hover:border-primary transition-all duration-700 shadow-2xl relative overflow-hidden group/card1">
                            <h5 className="text-[14px] font-black text-primary uppercase tracking-[1em] mb-8 italic border-b-2 border-primary/10 pb-4 flex items-center gap-4 gold-glow">
                               <Radar className="size-6 animate-neural" /> Autonomous Decision
                            </h5>
                            <div className="text-lg text-emerald-400 font-black italic whitespace-pre-wrap leading-relaxed h-[400px] overflow-y-auto scrollbar-hide selection:bg-primary">
                               {chainResult.autonomousDecision || "Executing Blitzkrieg sequence via Node 22..."}
                            </div>
                         </Card>
                         <Card className="bg-black/80 border-2 border-white/5 p-8 rounded-3xl hover:border-primary transition-all duration-700 shadow-2xl relative overflow-hidden group/card2">
                            <h5 className="text-[14px] font-black text-primary uppercase tracking-[1em] mb-8 italic border-b-2 border-primary/10 pb-4 flex items-center gap-4">
                               <Fingerprint className="size-6" /> Target DNA Match
                            </h5>
                            <pre className="text-sm text-gray-400 font-black italic whitespace-pre-wrap h-[400px] overflow-y-auto scrollbar-hide">
                               {JSON.stringify(chainResult.reconIntel.intelligenceProfile, null, 2)}
                            </pre>
                         </Card>
                      </div>

                      <div className="p-10 rounded-[4rem] bg-emerald-600/10 border-4 border-emerald-500/30 flex items-center gap-10 group/strat shadow-9xl relative overflow-hidden mt-auto">
                         ...
                         <div>
                            <h4 className="text-xl font-black text-emerald-500 uppercase tracking-[0.6em] mb-2 italic">Strike_Strategy_v53.5</h4>
                   ...
                   <div className="h-full flex flex-col items-center justify-center text-center opacity-10 gap-16 py-40">
                      ...
                      <div className="space-y-6">
                         <h3 className="text-5xl md:text-9xl font-black uppercase tracking-[1.5em] text-white italic gold-glow leading-none">Scanning</h3>
                         <p className="text-xl md:text-3xl font-bold italic text-gray-500 uppercase tracking-widest">Establishing autonomous neural link to target matrix.</p>
                      </div>
                   </div>
                 )}
              </CardContent>
              <div className="p-8 border-t-2 border-white/5 mt-auto flex justify-between items-center opacity-30 text-[10px] font-black uppercase tracking-[2em] italic">
                 <span>SUPREME_HIERARCHY_KILL_CHAIN_v53_AL_GHAZALI_ROOT</span>
        ...
        <div className="mt-auto relative z-10 flex justify-center items-center gap-16 opacity-40 text-[12px] md:text-[18px] font-black uppercase tracking-[2em] md:tracking-[6em] italic text-white drop-shadow-xl pb-12">
            <span>AL-MUIZZ AUTONOMOUS KILL-CHAIN v53.5</span>
