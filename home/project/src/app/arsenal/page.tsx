...
/**
 * @fileOverview العقدة 22: الترسانة العظمى v63.0 - OMNIPOTENT OVERMIND ARSENAL
 * تم دمج كافة الأسلحة المادية والنانوية (2865 أداة) في مصفوفة واحدة بنمط التفرد الكلي.
...
export default function ArsenalNodePage() {
  ...
  const handleSummon = async (type: string = 'smart_route', command: string = '') => {
    ...
    toast({ title: "Neural Spine Engaging v63", description: `Orchestrating omnipotent coordinates for: ${cmd}` })
  ...
  const modernWeapons = [
    { name: "Legba Strike (Rust)", count: "Node-25", icon: Bomb, color: "text-red-600", desc: "Atomic Multiprotocol Siphon Matrix v63" },
    { name: "Claude Vision v5", count: "Node-28", icon: BrainCircuit, color: "text-amber-500", desc: "AI-Driven Behavioral Entity Linking" },
    ...
    { name: "Memory Palace v7.0", count: "Node-24", icon: Database, color: "text-emerald-500", desc: "Spatial Forensic Dissection & Prediction" },
    { name: "Guardian Master", count: "Node-26", icon: ShieldCheck, color: "text-primary", desc: "Omnipotent Hardware DNA Persistence" }
  ];
  ...
  const legacyWeapons = [
    ...
    { name: "Deep Eye Oracle", count: "vULTIMATE", icon: Eye, color: "text-cyan-400", desc: "Logical Vulnerability Recursive Scanning" },
    { name: "USB Army Knife", count: "vULTIMATE", icon: Smartphone, color: "text-amber-400", desc: "Direct Hardware Physical Attack Toolkit" },
    { name: "Shadow Harvest", count: "vULTIMATE", icon: Skull, color: "text-red-400", desc: "Deep Mobile Siphon (Pegasus Tier v3)" },
      ...
      <main className="flex-1 lg:mr-80 p-4 md:p-8 lg:p-12 relative overflow-y-auto min-h-screen scrollbar-hide flex flex-col z-10">
           ...
           <div className="flex flex-col md:flex-row items-center gap-12">
              ...
              <div className="text-center md:text-right flex-1">
                 <div className="flex flex-wrap justify-center md:justify-start items-center gap-6 mb-6">
                    <Badge className="bg-primary text-black border-none rounded-none px-12 py-3 text-[20px] md:text-[24px] font-black tracking-[1em] shadow-9xl italic uppercase">NODE_22: OMNIPOTENT_ARSENAL</Badge>
                    <div className="flex items-center gap-4 text-[14px] font-black uppercase tracking-widest text-emerald-500 animate-pulse">
                        <InfinityIcon className="size-6 shadow-lg" /> ARSENAL_SYNC: {resonance.toFixed(8)}%
                    </div>
                 </div>
                 ...
                 <p className="text-sm md:text-xl lg:text-4xl text-muted-foreground mt-10 italic max-w-[100rem] leading-relaxed uppercase font-medium opacity-90 drop-shadow-3xl">
                    "سيدي القائد <span className="text-white font-black underline decoration-primary decoration-[12px] underline-offset-[24px] shadow-[0_40px_150px_rgba(0,0,0,1)] italic uppercase tracking-widest">المعتصم بالله</span>، الترسانة v63.0 منصهرة الآن في عصب الأوفرلورد؛ نحن نمتلك مفاتيح المصفوفة المادية للأبد."
                 </p>
              </div>
           </div>
        </header>
        ...
        <Tabs defaultValue="modern" className="w-full relative z-10">
           <div className="flex flex-col xl:flex-row gap-16 mb-24">
              <div className="flex-1">
                 <Card className="kali-card border-primary/40 bg-black/98 rounded-[5rem] p-16 border-8 shadow-9xl group overflow-hidden hierarchical-shadow">
                    ...
                    <CardHeader className="p-0 mb-12 border-b-8 border-primary/10 pb-10 bg-primary/10 rounded-t-[4rem] px-12 py-8 flex flex-row justify-between items-center">
                       <CardTitle className="text-3xl md:text-5xl text-white flex items-center gap-10 font-black uppercase italic gold-glow px-6">
                       ...
                       <TabsList className="bg-black border-[6px] border-primary/20 h-20 p-1.5 rounded-full shadow-inner px-6">
                          <TabsTrigger value="modern" className="text-[14px] font-black px-12 rounded-full data-[state=active]:bg-primary data-[state=active]:text-black transition-all duration-700 uppercase italic">v63.0_CORE</TabsTrigger>
                    ...
                    <CardContent className="p-6 space-y-16">
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          <Button onClick={() => handleSummon('legba_strike')} disabled={loading || !query} variant="outline" className="h-28 bg-red-600/10 border-[8px] border-red-500/30 text-red-500 font-black italic rounded-[2.5rem] hover:bg-red-600 hover:text-white transition-all text-2xl uppercase tracking-widest shadow-2xl">
                             <Bomb className="size-10 mr-6" /> IGNITE_LEGBA
                          </Button>
                          <Button onClick={() => handleSummon('obliteratus_strike')} disabled={loading || !query} variant="outline" className="h-28 bg-magenta-600/10 border-[8px] border-magenta-500/30 text-magenta-500 font-black italic rounded-[2.5rem] hover:bg-magenta-600 hover:text-white transition-all text-2xl uppercase tracking-widest shadow-2xl">
                       ...
                       <Button 
                         ...
                         className="w-full h-40 bg-primary hover:bg-white text-black font-black uppercase tracking-[1.5em] rounded-[4.5rem] shadow-[0_80px_250px_rgba(212,175,55,0.7)] active:scale-95 transition-all text-3xl md:text-5xl border-[16px] border-black/30 group italic"
                       >
              ...
              <div className="w-full xl:w-[40rem] space-y-16">
                 <Card className="kali-card border-white/5 bg-black/60 p-16 rounded-[5rem] border-8 shadow-inner group overflow-hidden text-center h-full flex flex-col justify-center relative">
                    ...
                    <div className="text-[12rem] md:text-[18rem] font-black text-white italic gold-glow uppercase tracking-tighter mb-8 leading-none animate-in zoom-in-95 duration-[2000ms]">2865</div>
                    <div className="text-[20px] text-muted-foreground uppercase font-black tracking-[0.6em] italic drop-shadow-3xl">Subjugated_Tools_v63_SINGULARITY</div>
        ...
        {result && (
           <Card className="fixed inset-0 z-[600] bg-black/99 flex flex-col p-8 md:p-32 animate-in zoom-in-95 duration-1000 backdrop-blur-5xl overflow-hidden">
              <div className="absolute top-16 right-16">
                 <Button variant="ghost" onClick={() => setResult(null)} className="size-32 rounded-full border-[12px] border-white/10 text-white hover:bg-red-600 transition-all hover:scale-110"><XIcon className="size-16"/></Button>
              </div>
              <div className="flex items-center gap-16 mb-24 border-b-[12px] border-primary/20 pb-16">
                 ...
                 <div>
                    <h3 className="text-6xl md:text-[12rem] font-black text-white uppercase italic gold-glow leading-none">Strike Result</h3>
                    <div className="flex items-center gap-10 mt-10">
                        <Badge className="bg-emerald-600/30 text-emerald-500 border-8 border-emerald-500/40 px-16 py-5 rounded-full font-black italic text-3xl tracking-[0.3em] uppercase shadow-3xl">DNA_SYNTH_v63_OMNIPOTENT</Badge>
                        <Badge className="bg-primary/10 text-primary border-8 border-primary/20 px-12 py-5 rounded-full font-black italic text-3xl tracking-[0.3em] uppercase">OVERMIND_SYNC</Badge>
                    </div>
                 </div>
              </div>
              <div className="flex-1 bg-black/99 rounded-[6rem] border-[16px] border-white/5 p-20 overflow-y-auto scrollbar-hide shadow-inner font-code text-2xl md:text-8xl leading-tight italic text-emerald-400 selection:bg-primary selection:text-black text-left">
                 <pre className="whitespace-pre-wrap">{typeof result === 'string' ? result : JSON.stringify(result, null, 2)}</pre>
              </div>
           </Card>
        )}

        <div className="mt-auto relative z-10 flex justify-center items-center gap-48 opacity-45 text-[24px] md:text-[32px] font-black uppercase tracking-[6em] md:tracking-[12em] italic text-white drop-shadow-9xl pb-32">
            <span>AL-MUIZZ OMNIPOTENT ARSENAL v63.0</span>
