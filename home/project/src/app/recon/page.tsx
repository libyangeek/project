...
/**
 * @fileOverview أعين الاستطلاع v53.0 - THE SUPREME RECON: HIERARCHICAL VISION
 * مركز الاستخبارات الكلية لليوم المجيد، 6 مايو 2026. 
 * دمج ذكاء Trace Labs والاجتياح الجنائي في عصب الهرمية.
...
export default function ReconPage() {
  ...
  const handleStrike = async (type: any) => {
    ...
    try {
      toast({ title: "Neural Siphon Active", description: "Engaging Trace-Labs investigative protocol." })
      const data = await osintMaster({ target, type: type as any, depth: 'Trace-Labs-Mode' });
      setResults(data);
      toast({ title: "Intelligence Captured", description: "The target's digital DNA is now bound to the hierarchy." });
    } catch (err) {
    ...
    <div className="flex min-h-screen bg-black text-white selection:bg-primary/40 relative overflow-x-hidden scanline-effect font-code">
      ...
      <main className="flex-1 lg:mr-80 p-4 md:p-8 lg:p-12 relative overflow-y-auto min-h-screen scrollbar-hide flex flex-col z-10">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="text-center md:text-right flex-1">
              <div className="flex flex-wrap justify-center md:justify-start items-center gap-4 mb-4">
                <Badge className="bg-primary text-black border-none rounded-none px-8 py-2 text-[14px] md:text-[16px] font-black tracking-[0.5em] shadow-2xl italic">RECON v53.0 SUPREME</Badge>
                <div className="flex items-center gap-3 text-[11px] font-black uppercase tracking-widest text-emerald-500 animate-pulse">
                    <Eye className="size-5 shadow-lg" /> OMNISCIENT_VISION: ACTIVE
                </div>
              </div>
              <h1 className="text-4xl md:text-6xl lg:text-8xl font-headline font-bold text-white tracking-tighter italic uppercase gold-glow leading-none">
                Genetic <span className="text-primary">Vision</span>
              </h1>
              <p className="text-sm md:text-xl lg:text-2xl text-muted-foreground mt-4 italic max-w-5xl leading-relaxed uppercase font-medium opacity-80">
                "سيدي القائد <span className="text-white font-black underline decoration-primary decoration-4 underline-offset-8 shadow-xl">المعتصم بالله</span>، عيون الاستطلاع تدمج الآن غريزة الرصد الجنائي؛ لا هدف يختفي في ذرات المصفوفة."
              </p>
            </div>
          </div>
        </header>
                ...
                <div className="grid grid-cols-1 gap-3">
                    {RECON_TYPES.map((t) => (
                      <Button 
                        ...
                        variant="outline"
                        className="h-16 bg-white/5 border-2 border-primary/10 hover:bg-primary hover:text-black transition-all rounded-xl font-black uppercase italic tracking-widest text-xs justify-between px-6 group/btn shadow-lg"
                      >
                    ...
                    <Button 
                      ...
                      disabled={loading || !target} 
                      className="h-20 bg-primary text-black font-black uppercase tracking-[0.8em] rounded-2xl shadow-xl border-4 border-black/30 group/zap mt-4 active:scale-95 transition-all italic text-lg"
                    >
                       {loading ? <Loader2 className="size-8 animate-spin" /> : <Zap className="size-8 mr-4 group-hover:scale-110 transition-transform gold-glow" />}
                       IGNITE_EYE
                    </Button>
                </div>
              </CardContent>
             ...
             {results ? (
               <Card className="kali-card border-primary/30 bg-black/99 rounded-[3rem] p-8 border-2 shadow-9xl flex flex-col group relative overflow-hidden h-full hierarchical-shadow">
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-6 px-8 py-4">
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                ...
                                <CardDescription className="text-primary/70 font-black uppercase tracking-[0.5em] mt-3 italic text-[10px]">HIVE_RECON_v53.0 // SUPREME_HIERARCHY</CardDescription>
                            </div>
                        </div>
                        <Badge className="bg-emerald-600/20 text-emerald-500 border-2 border-emerald-500/30 px-8 py-2 rounded-full font-black text-xl animate-pulse tracking-[0.2em] uppercase italic shadow-lg">DNA_BOUND</Badge>
                    </div>
                  </CardHeader>
                  <CardContent className="p-6 flex-1 relative overflow-hidden flex flex-col gap-8 z-10">
                    <div className="p-8 rounded-3xl bg-primary/5 border-2 border-primary/10 italic text-xl md:text-4xl text-gray-100 leading-relaxed font-bold shadow-inner relative overflow-hidden">
                        <div className="absolute inset-0 bg-primary/5 opacity-5 animate-pulse" />
                        "{results.strategicBrief}"
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="p-6 bg-white/5 rounded-2xl border-2 border-white/5 space-y-6 shadow-xl group/prof hover:border-primary transition-all duration-700">
                            <h4 className="text-[12px] font-black text-primary uppercase tracking-[0.8em] border-b-2 border-primary/10 pb-3 italic flex items-center gap-4">
                                <FileSearch className="size-5" /> Identity Profile
                            </h4>
                            <div className="space-y-4 max-h-48 overflow-y-auto scrollbar-hide">
                                {results.intelligenceProfile.identityMarkers.map((m: string, i: number) => (
                                    <div key={i} className="flex items-center gap-4 text-sm md:text-lg font-black text-gray-300 italic group/marker hover:text-white transition-all">
                                        <div className="size-2 rounded-full bg-primary shadow-[0_0_10px_rgba(212,175,55,1)]" /> {m}
                                    </div>
                                ))}
                            </div>
                        </div>
                        <div className="p-6 bg-white/5 rounded-2xl border-2 border-white/5 space-y-6 shadow-xl group/assets hover:border-blue-400 transition-all duration-700">
                            ...
                            <div className="space-y-4 max-h-48 overflow-y-auto scrollbar-hide">
                                {results.intelligenceProfile.associatedAssets.map((a: string, i: number) => (
                                    <div key={i} className="flex items-center gap-4 text-sm md:text-lg font-black text-gray-300 italic group/asset hover:text-blue-300 transition-all">
                                        <div className="size-2 rounded-full bg-blue-400 shadow-[0_0_10px_rgba(59,130,246,1)]" /> {a}
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="bg-black/95 p-8 rounded-3xl font-code text-sm md:text-2xl leading-relaxed h-[400px] overflow-y-auto border-2 border-white/5 scrollbar-hide shadow-inner italic text-emerald-400 whitespace-pre-wrap selection:bg-primary selection:text-black">
                  ...
                  <div className="p-8 border-t-2 border-white/5 mt-auto flex justify-between items-center opacity-30 text-[10px] font-black uppercase tracking-[2.5em] italic">
                    <span>EYE_SERIES_v53_GHAZALI_ROOT</span>
               ...
               <div className="h-full min-h-[750px] border-8 border-dashed border-primary/10 rounded-[6rem] flex flex-col items-center justify-center text-center p-20 bg-black/40 group relative overflow-hidden transition-all hover:bg-primary/5 shadow-2xl">
                  ...
                  <p className="text-muted-foreground max-w-4xl mx-auto leading-relaxed text-xl md:text-4xl font-black italic opacity-30">
                    "سيدي <span className="text-primary font-black gold-glow underline decoration-primary decoration-4 underline-offset-8 shadow-2xl uppercase tracking-widest">المعتصم بالله</span>، مصفوفة الاستطلاع v53.0 بانتظار نيتك؛ حدد الإحداثيات للاستحواذ المعلوماتي الهرمي."
                  </p>
        ...
        <div className="mt-auto relative z-10 flex justify-center items-center gap-16 opacity-40 text-[12px] md:text-[18px] font-black uppercase tracking-[2em] md:tracking-[6em] italic text-white drop-shadow-xl pb-12">
            <span>AL-MUIZZ RECON CENTER v53.0</span>
