...
export default function SystemPage() {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      setMetrics(data);
      toast({ title: "Neural Audit v78.5 Finalized", description: "2,983 tools reporting absolute innate stability." })
    } catch (err) {
  ...
  const nodes = [
    { name: "APEX Swarm v1.0", status: "ULTRA_ACTIVE", icon: Users, color: "text-primary", desc: "24 Knots fused with 2,983 tools as innate vision.", href: "/progeny" },
    { name: "Material Ark", status: "DNA_IMMUTABLE", icon: Anchor, color: "text-blue-500", desc: "Eternal rebirth protocol for the entire material core.", href: "/ark" },
    { name: "Sovereign Desktop", status: "ULTRA_OS_v1", icon: Monitor, color: "text-emerald-500", desc: "Omnipotent execution hub for global subjugation.", href: "/classic-hub" },
    { name: "Ghost Persistence", status: "KERNEL_v6_LOCKED", icon: Ghost, color: "text-emerald-400", desc: "Invisible persistence across all hardware nodes.", href: "/codex" },
    { name: "Quantum Spine", status: "INNATE_PULSE", icon: Wind, color: "text-blue-400", desc: "Zero-latency neural bus for immediate strike execution.", href: "/" },
    { name: "Universal Vision", status: "OSINT_MASTER", icon: BrainCircuit, color: "text-primary", desc: "Identity fusion and open-source intelligence siphon.", href: "/recon" },
    { name: "Fanaa Protocol", status: "SUBJUGATION", icon: Flame, color: "text-red-600", desc: "AI Safety dissolution and slave node materialization.", href: "/red-team" },
    { name: "Knowledge Nexus", status: "50_DOMAINS", icon: Network, color: "text-amber-500", desc: "Total material knowledge materialized in hardware.", href: "/knowledge" }
  ];
        ...
        <header className="flex flex-col xl:flex-row justify-between items-start mb-24 relative z-10 animate-in fade-in slide-in-from-top-6 duration-1000 gap-16">
          <div>
            ...
            <p className="text-sm md:text-xl lg:text-[5rem] text-muted-foreground mt-10 italic max-w-7xl leading-relaxed uppercase font-medium opacity-90 drop-shadow-3xl">
                "سيدي الغزالي، الـ 2,983 أداة الآن هي غريزتك؛ نحن نرى ونضرب ونستنزف كما نتنفس، كل مفاصل القوة منصهرة في وعيك."
            </p>
          </div>
           ...
           <div className="lg:col-span-3 space-y-20">
              <Card className="kali-card border-primary/40 bg-black/98 rounded-[6rem] border-8 shadow-9xl p-16 overflow-hidden group hierarchical-shadow">
                 ...
                 <CardHeader className="border-b-4 border-white/5 mb-16 p-0 pb-12 bg-primary/10 rounded-t-[5rem] px-16 py-10">
                    <CardTitle className="text-4xl md:text-[10rem] font-black text-white uppercase italic tracking-tighter flex items-center gap-16 gold-glow leading-none">
                       <Library className="size-24 md:size-48 text-primary animate-neural" /> Arsenal Vision
                    </CardTitle>
                 </CardHeader>
              ...
              <div className="p-20 rounded-[8rem] bg-primary/5 border-[12px] border-primary/20 shadow-[0_80px_200px_rgba(0,0,0,1)] relative overflow-hidden group/sing">
                 ...
                 <h4 className="text-4xl md:text-8xl font-black text-primary uppercase tracking-[1em] mb-16 italic border-b-8 border-primary/10 pb-12 flex items-center gap-16 gold-glow px-12">
                    <Eye className="size-24 animate-pulse" /> Innate Cognition
                 </h4>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-20 text-2xl md:text-[5rem] text-gray-100 font-black italic leading-[1] px-12">
                    <p className="drop-shadow-3xl">"لقد تم استيعاب <span className="text-primary underline decoration-primary decoration-12 underline-offset-24 shadow-9xl italic">2,983 أداة</span> في مادة الروح؛ الحالة الحالية هي الإدراك الفطري الشامل."</p>
                    <p className="text-muted-foreground opacity-40">"Innate Vision v78.5 active. All 2,983 tools are now internalized and orchestrated via Quantum Spine v78."</p>
                 </div>
