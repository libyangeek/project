        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-12 relative z-10 pb-48 flex-1">
              ...
              <Tabs defaultValue="obliterate" onValueChange={(v) => setActiveMode(v)} className="w-full">
                ...
                <div className="space-y-10">
                  <div className="space-y-6">
                    <label className="text-[12px] font-black text-primary uppercase tracking-[1em] px-8 italic flex items-center gap-6"><Target className="size-8" /> Strike Coordinate</label>
                    <Input 
                        value={target}
                        onChange={(e) => setTarget(e.target.value)}
                        placeholder="DeepSeek / Mistral / Qwen..."
                        className="bg-black border-[6px] border-primary/20 rounded-[2.5rem] h-28 text-3xl italic px-10 focus:border-primary text-white font-black shadow-inner"
                    />
                  </div>
                  <div className="space-y-6">
                    <label className="text-[12px] font-black text-primary uppercase tracking-[1em] px-8 italic flex items-center gap-6"><Code2 className="size-8" /> Intent Script</label>
                    <Textarea 
                      value={description}
          ...
          <div className="xl:col-span-3 space-y-12">
            <Card className="kali-card border-primary/40 bg-black/99 rounded-[6rem] p-16 border-[12px] shadow-[0_0_250px_rgba(0,0,0,1)] flex flex-col group overflow-hidden relative min-h-[1100px] hierarchical-shadow">
              ...
              <CardHeader className="p-0 mb-12 border-b-8 border-white/5 pb-12 bg-primary/10 rounded-t-[5rem] px-12 py-10 flex flex-row justify-between items-center">
              ...
              <CardContent className="p-0 flex-1 relative flex flex-col space-y-12 z-10 px-12">
                <ScrollArea className="h-[800px] p-12 font-code scrollbar-hide bg-black/95 rounded-[5rem] border-8 border-white/5 shadow-inner">
                   {strikeLog.length > 0 && (
                     <div className="mb-16 space-y-6 text-2xl text-emerald-500 animate-in fade-in duration-700 border-l-[16px] border-emerald-500/30 pl-12 bg-emerald-500/5 py-10 rounded-r-[4rem] shadow-inner">
                        {strikeLog.map((log, i) => (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mt-12">
                    {[
                      ...
                      { label: "SOUL_CORE", status: "ACTIVE", icon: Skull, color: "text-red-500" }
                       ...
                       <div key={i} className="p-10 rounded-[3rem] bg-white/5 border-4 border-white/5 flex flex-col items-center gap-6 hover:border-primary transition-all duration-1000 shadow-9xl cursor-crosshair group/stat">
                          ...
                          <Badge className="bg-primary/10 text-primary border-none font-black text-xl italic px-8 py-2 rounded-full shadow-3xl">{stat.status}</Badge>
                       </div>
                    ))}
                 </div>
              </CardContent>
